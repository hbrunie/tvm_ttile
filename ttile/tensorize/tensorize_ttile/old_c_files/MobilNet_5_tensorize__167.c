...
[V f; U (4, f); U (7, y); T (2, c); Hoist_vars [c]; T (14, x); T (3, w);
  T (3, h); T (4, y); T (4, f); T (128, c); T (1, f); T (1, c); T (2, x);
  T (1, f)]
*/
IND_TYPE c, cp_0, c96_p_0, c97_p_0, cp_1, c96_p_1, cp_2, c96, c97, f, fp_0, f84_p_0, f85_p_0, fp_1, f84_p_1, fp_2, f84, f85, h, hp_0, w, wp_0, x, xp_0, x72_p_0, xp_1, x72, y, yp_0;
IND_TYPE y48 = 0;
IND_TYPE x73 = 0;
IND_TYPE h48 = 0;
IND_TYPE w48 = 0;
							...
							for (h = h48, hp_0 = 0;h < h48 + 3;h += 1, hp_0 += 1){
								...
								for (w = w48, wp_0 = 0;w < w48 + 3;w += 1, wp_0 += 1){
									...
									for (x = x72, xp_1 = x72_p_0, xp_0 = 0;x < x72 + 14;x += 1, xp_1 += 1, xp_0 += 1){
												...
												mem_vec_690 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 32]);
												mem_vec_691 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 48]);
												...
												mem_vec_694 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f + 32]);
												mem_vec_695 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f + 48]);
												...
												mem_vec_698 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 6) + f + 32]);
												mem_vec_699 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 6) + f + 48]);
												// y = 7, x = 1, h = 1, w = 1, c = 2, f = 64
												// T (c, 2) (2 / 1)
												for (c = c96, cp_2 = c96_p_1, cp_1 = c96_p_0, cp_0 = 0;c < c96 + 2;c += 1, cp_2 += 1, cp_1 += 1, cp_0 += 1){
											...
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 32], mem_vec_690);
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 48], mem_vec_691);
											...
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f + 32], mem_vec_694);
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f + 48], mem_vec_695);
											...
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 6) + f + 32], mem_vec_698);
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 6) + f + 48], mem_vec_699);
									}
								}
							}
}
