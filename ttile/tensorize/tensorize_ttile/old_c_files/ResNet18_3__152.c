...
*/
IND_TYPE c, cp_0, c507_p_0, cp_1, c507, f, fp_0, x, xp_0, x836_p_0, x837_p_0, x838_p_0, xp_1, x836_p_1, x837_p_1, xp_2, x836_p_2, xp_3, x836, x837, x838, y, yp_0, y667_p_0, y668_p_0, yp_1, y667_p_1, yp_2, y667, y668;
...
for (y668 = y669, y668_p_0 = 0;
	...
	for (x838 = x839, x838_p_0 = 0;
		...
		// T (y, 2) (28 / 14)
		for (y667 = y668, y667_p_1 = y668_p_0, y667_p_0 = 0;
			y667 < y668 + 28;
			y667 += 14, y667_p_1 += 14, y667_p_0 += 14){
			// y = 14, x = 7, h = 1, w = 1, c = 64, f = 64
			// T (x, 7) (7 / 1)
			for (x837 = x838, x837_p_1 = x838_p_0, x837_p_0 = 0;
				x837 < x838 + 7;
				x837 += 1, x837_p_1 += 1, x837_p_0 += 1){
				...
				for (f = f329, fp_0 = 0;
					...
					// T (y, 1) (14 / 14)
					for (y = y667, yp_2 = y667_p_1, yp_1 = y667_p_0, yp_0 = 0;
						y < y667 + 14;
						y += 14, yp_2 += 14, yp_1 += 14, yp_0 += 14){
						// y = 14, x = 1, h = 1, w = 1, c = 64, f = 32
						// T (x, 1) (1 / 1)
						for (x836 = x837, x836_p_2 = x837_p_1, x836_p_1 = x837_p_0, x836_p_0 = 0;
							x836 < x837 + 1;
							x836 += 1, x836_p_2 += 1, x836_p_1 += 1, x836_p_0 += 1){
							// y = 14, x = 1, h = 1, w = 1, c = 64, f = 32
							// T (c, 2) (64 / 32)
							for (c507 = c508, c507_p_0 = 0;
								...
								// T (x, 1) (1 / 1)
								for (x = x836, xp_3 = x836_p_2, xp_2 = x836_p_1, xp_1 = x836_p_0, xp_0 = 0;
									x < x836 + 1;
									x += 1, xp_3 += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_4250 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f]);
											mem_vec_4251 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16]);
											mem_vec_4252 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
											mem_vec_4253 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
											mem_vec_4254 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f]);
											mem_vec_4255 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16]);
											// y = 14, x = 1, h = 1, w = 1, c = 32, f = 32
											// T (c, 32) (32 / 1)
											for (c = c507, cp_1 = c507_p_0, cp_0 = 0;
												...
												vec_38 = _mm512_fmadd_ps(vec_39, vec_2, mem_vec_4252);
												mem_vec_4252 = vec_38;



												vec_40 = _mm512_fmadd_ps(vec_39, vec_4, mem_vec_4253);
												mem_vec_4253 = vec_40;
												scal_13 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 13 + h) + c];
												vec_42 = _mm512_set1_ps(scal_13);


												vec_41 = _mm512_fmadd_ps(vec_42, vec_2, mem_vec_4254);
												mem_vec_4254 = vec_41;



												vec_43 = _mm512_fmadd_ps(vec_42, vec_4, mem_vec_4255);
												mem_vec_4255 = vec_43;
											}
										...
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f], mem_vec_4250);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16], mem_vec_4251);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_4252);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_4253);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f], mem_vec_4254);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16], mem_vec_4255);
								}
							}
						}
					}
