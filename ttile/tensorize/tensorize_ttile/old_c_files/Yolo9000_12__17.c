								...
								for (w = w12, wp_0 = 0;
									...
									for (x = x18, xp_1 = x18_p_0, xp_0 = 0;
										...
										x += 1, xp_1 += 1, xp_0 += 1){
												...
												mem_vec_250 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f]);
												mem_vec_251 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16]);
												// y = ph_y, x = 1, h = 1, w = 1, c = 32, f = 32
												// T (c, 32) (32 / 1)
												for (c = c18, cp_1 = c18_p_0, cp_0 = 0;
													c < c18 + 32;
											...
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f], mem_vec_250);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16], mem_vec_251);
									}
								}
							}
						}
