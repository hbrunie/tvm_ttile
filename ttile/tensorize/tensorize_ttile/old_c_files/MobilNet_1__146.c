...
/*
[V f; U (2, f); U (14, y); T (32, c); Hoist_vars [c]; T (56, x); T (3, w);
  T (3, h); T (1, x); T (8, y); T (1, c); T (2, x)]
*/
IND_TYPE c, cp_0, c72_p_0, cp_1, c72, h, hp_0, w, wp_0, x, xp_0, x96_p_0, x97_p_0, xp_1, x96_p_1, xp_2, x96, x97, y, yp_0;
		...
		for (y = y48, yp_0 = 0;
			...
			for (x96 = x97, x96_p_1 = x97_p_0, x96_p_0 = 0;
				...
				for (h = h48, hp_0 = 0;
					h < h48 + 3;
					h += 1, hp_0 += 1){
					// y = 14, x = 56, h = 1, w = 3, c = 32, f = 32
					// T (w, 3) (3 / 1)
					for (w = w48, wp_0 = 0;
						w < w48 + 3;
						w += 1, wp_0 += 1){
						// y = 14, x = 56, h = 1, w = 1, c = 32, f = 32
						// T (x, 56) (56 / 1)
						for (x = x96, xp_2 = x96_p_1, xp_1 = x96_p_0, xp_0 = 0;
							...
							x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
									...
									mem_vec_430 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f]);
									mem_vec_431 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16]);
									mem_vec_432 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
									mem_vec_433 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
									mem_vec_434 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f]);
									mem_vec_435 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16]);
									...
									for (c = c72, cp_1 = c72_p_0, cp_0 = 0;
										...
										vec_41 = _mm512_fmadd_ps(vec_42, vec_2, mem_vec_434);
										mem_vec_434 = vec_41;



										vec_43 = _mm512_fmadd_ps(vec_42, vec_4, mem_vec_435);
										mem_vec_435 = vec_43;
									}
								...
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f], mem_vec_430);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16], mem_vec_431);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_432);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_433);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f], mem_vec_434);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16], mem_vec_435);
						}
					}
				}
			}
