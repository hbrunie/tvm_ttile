							...
							for (h = h100, hp_0 = 0;h < h100 + 3;h += 1, hp_0 += 1){
								...
								for (w = w100, wp_0 = 0;w < w100 + 3;w += 1, wp_0 += 1){
									...
									for (x = x150, xp_1 = x150_p_0, xp_0 = 0;x < x150 + 1;x += 1, xp_1 += 1, xp_0 += 1){
												...
												mem_vec_1972 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f]);
												mem_vec_1973 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f + 16]);
												// y = ph_y, x = 1, h = 1, w = 1, c = 256, f = 32
												// T (c, 256) (256 / 1)
											...
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f], mem_vec_1972);
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f + 16], mem_vec_1973);
									}
								}
							}
}
