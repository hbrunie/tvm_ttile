					...
					for (h = h468, hp_0 = 0;h < h468 + 3;h += 1, hp_0 += 1){
						...
						for (w = w488, wp_0 = 0;w < w488 + 3;w += 1, wp_0 += 1){
							...
							for (x = x976, xp_2 = x976_p_1, xp_1 = x976_p_0, xp_0 = 0;x < x976 + 4;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_9262 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f]);
										mem_vec_9263 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16]);
									...
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f], mem_vec_9262);
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16], mem_vec_9263);
							}
						}
					}
}
