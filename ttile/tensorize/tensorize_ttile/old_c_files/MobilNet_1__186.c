...
[V f; U (2, f); ULambda y; T (4, c); Hoist_vars [c]; T (16, x); T (3, w);
  ...
  T (7, x)]
*/
IND_TYPE c, cp_0, c558_p_0, cp_1, c558, h, hp_0, w, wp_0, x, xp_0, x744_p_0, x745_p_0, xp_1, x744_p_1, xp_2, x744, x745, y, yp_0;
		...
		c558 += 4, c558_p_0 += 4){
			for (y = y372, yp_0 = 0;
				...
				for (x744 = x745, x744_p_1 = x745_p_0, x744_p_0 = 0;
					x744 < x745 + 16;
					x744 += 16, x744_p_1 += 16, x744_p_0 += 16){
					// y = ph_y, x = 16, h = 3, w = 3, c = 4, f = 32
					// T (h, 3) (3 / 1)
					for (h = h352, hp_0 = 0;
						h < h352 + 3;
						...
						for (w = w372, wp_0 = 0;
							...
							for (x = x744, xp_2 = x744_p_1, xp_1 = x744_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_6404 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 8) + f]);
										mem_vec_6405 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 8) + f + 16]);
										mem_vec_6406 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f]);
										mem_vec_6407 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16]);
										...
										for (c = c558, cp_1 = c558_p_0, cp_0 = 0;
											...
											vec_26 = _mm512_fmadd_ps(vec_27, vec_2, mem_vec_6404);
											mem_vec_6404 = vec_26;



											vec_28 = _mm512_fmadd_ps(vec_27, vec_4, mem_vec_6405);
											mem_vec_6405 = vec_28;
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 8) + f], mem_vec_6404);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 8) + f + 16], mem_vec_6405);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f], mem_vec_6406);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16], mem_vec_6407);
							}
						}
					}
				}
			}
			for (y = y372 + 70, yp_0 = 0;
				y < y372 + 70 + 42;
				y += 14, yp_0 += 14){
				// y = ph_y, x = 16, h = 3, w = 3, c = 4, f = 32
				// T (x, 1) (16 / 16)
						...
						for (w = w372, wp_0 = 0;
							...
							for (x = x744, xp_2 = x744_p_1, xp_1 = x744_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_6428 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f]);
										mem_vec_6429 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16]);
										mem_vec_6430 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f]);
										mem_vec_6431 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16]);
										mem_vec_6432 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
										mem_vec_6433 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
										mem_vec_6434 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f]);
										mem_vec_6435 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16]);
										// y = ph_y, x = 1, h = 1, w = 1, c = 4, f = 32
										// T (c, 4) (4 / 1)
										for (c = c558, cp_1 = c558_p_0, cp_0 = 0;
											c < c558 + 4;
											...
											vec_32 = _mm512_fmadd_ps(vec_33, vec_2, mem_vec_6428);
											mem_vec_6428 = vec_32;



											vec_34 = _mm512_fmadd_ps(vec_33, vec_4, mem_vec_6429);
											mem_vec_6429 = vec_34;
											scal_11 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 11 + h) + c];
											vec_36 = _mm512_set1_ps(scal_11);


											vec_35 = _mm512_fmadd_ps(vec_36, vec_2, mem_vec_6430);
											mem_vec_6430 = vec_35;



											vec_37 = _mm512_fmadd_ps(vec_36, vec_4, mem_vec_6431);
											mem_vec_6431 = vec_37;
											scal_12 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 12 + h) + c];
											vec_39 = _mm512_set1_ps(scal_12);


											vec_38 = _mm512_fmadd_ps(vec_39, vec_2, mem_vec_6432);
											mem_vec_6432 = vec_38;



											vec_40 = _mm512_fmadd_ps(vec_39, vec_4, mem_vec_6433);
											mem_vec_6433 = vec_40;
											scal_13 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 13 + h) + c];
											vec_42 = _mm512_set1_ps(scal_13);


											vec_41 = _mm512_fmadd_ps(vec_42, vec_2, mem_vec_6434);
											mem_vec_6434 = vec_41;



											vec_43 = _mm512_fmadd_ps(vec_42, vec_4, mem_vec_6435);
											mem_vec_6435 = vec_43;
										}
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f], mem_vec_6428);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16], mem_vec_6429);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f], mem_vec_6430);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16], mem_vec_6431);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_6432);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_6433);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f], mem_vec_6434);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16], mem_vec_6435);
							}
						}
					}
				}
