...
/*
[V f; U (2, f); ULambda y; T (16, c); Hoist_vars [c]; T (112, x); T (3, w);
  T (3, h); T (1, x);
			...
			for (y = y196, yp_0 = 0;
				...
				for (x392 = x393, x392_p_1 = x393_p_0, x392_p_0 = 0;
					...
					for (h = h176, hp_0 = 0;
						h < h176 + 3;
						h += 1, hp_0 += 1){
						// y = ph_y, x = 112, h = 1, w = 3, c = 16, f = 32
						// T (w, 3) (3 / 1)
						for (w = w196, wp_0 = 0;
							w < w196 + 3;
							w += 1, wp_0 += 1){
							// y = ph_y, x = 112, h = 1, w = 1, c = 16, f = 32
							// T (x, 112) (112 / 1)
						...
						for (w = w196, wp_0 = 0;
							...
							for (x = x392, xp_2 = x392_p_1, xp_1 = x392_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_2564 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f]);
										mem_vec_2565 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16]);
										mem_vec_2566 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f]);
										mem_vec_2567 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16]);
										mem_vec_2568 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
										mem_vec_2569 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
										// y = ph_y, x = 1, h = 1, w = 1, c = 16, f = 32
										// T (c, 16) (16 / 1)
										for (c = c294, cp_1 = c294_p_0, cp_0 = 0;
											c < c294 + 16;
											...
											vec_35 = _mm512_fmadd_ps(vec_36, vec_2, mem_vec_2566);
											mem_vec_2566 = vec_35;



											vec_37 = _mm512_fmadd_ps(vec_36, vec_4, mem_vec_2567);
											mem_vec_2567 = vec_37;
											scal_12 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 12 + h) + c];
											vec_39 = _mm512_set1_ps(scal_12);


											vec_38 = _mm512_fmadd_ps(vec_39, vec_2, mem_vec_2568);
											mem_vec_2568 = vec_38;



											vec_40 = _mm512_fmadd_ps(vec_39, vec_4, mem_vec_2569);
											mem_vec_2569 = vec_40;
										}
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f], mem_vec_2564);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16], mem_vec_2565);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f], mem_vec_2566);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16], mem_vec_2567);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_2568);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_2569);
							}
						}
					}
				}
