							...
							for (w = w218, wp_0 = 0;
								...
								for (x = x496, xp_2 = x496_p_1, xp_1 = x496_p_0, xp_0 = 0;
									...
									x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_3840 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f]);
											mem_vec_3841 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16]);
											// y = ph_y, x = 1, h = 1, w = 1, c = 64, f = 32
											// T (c, 64) (64 / 1)
											for (c = c372, cp_1 = c372_p_0, cp_0 = 0;
												c < c372 + 64;
										...
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f], mem_vec_3840);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16], mem_vec_3841);
								}
							}
						}
					}
