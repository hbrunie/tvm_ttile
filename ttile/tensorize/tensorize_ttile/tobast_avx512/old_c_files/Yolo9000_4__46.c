...
/*
[V f; U (4, f); U (2, y); U (3, h); T (2, c); Hoist_vars [c]; T (17, x);
  T (3, w); T (8, x); T (68, y); T (2, f); T (32, c); T (1, x)]
*/
IND_TYPE c, cp_0, c108_p_0, cp_1, c108, f, fp_0, w, wp_0, x, xp_0, x142_p_0, x143_p_0, xp_1, x142_p_1, xp_2, x142, x143, y, yp_0;
...
for (x143 = x144, x143_p_0 = 0;
	...
	for (c108 = c109, c108_p_0 = 0;
		...
		for (f = f72, fp_0 = 0;
			f < f72 + 128;
			f += 64, fp_0 += 64){
			// y = 136, x = 136, h = 3, w = 3, c = 2, f = 64
			// T (y, 68) (136 / 2)
			for (y = y72, yp_0 = 0;
				y < y72 + 136;
				y += 2, yp_0 += 2){
				// y = 2, x = 136, h = 3, w = 3, c = 2, f = 64
				// T (x, 8) (136 / 17)
				for (x142 = x143, x142_p_1 = x143_p_0, x142_p_0 = 0;
					x142 < x143 + 136;
					x142 += 17, x142_p_1 += 17, x142_p_0 += 17){
					// y = 2, x = 17, h = 3, w = 3, c = 2, f = 64
					// T (w, 3) (3 / 1)
					for (w = w72, wp_0 = 0;
						w < w72 + 3;
						w += 1, wp_0 += 1){
						// y = 2, x = 17, h = 3, w = 1, c = 2, f = 64
						// T (x, 17) (17 / 1)
						for (x = x142, xp_2 = x142_p_1, xp_1 = x142_p_0, xp_0 = 0;
							x < x142 + 17;
							x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
									...
									for (c = c108, cp_1 = c108_p_0, cp_0 = 0;
										...
										vec_14 = _mm512_fmadd_ps(vec_15, vec_16, mem_vec_560);
										mem_vec_560 = vec_14;

										vec_18 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f + 16]);

										vec_17 = _mm512_fmadd_ps(vec_15, vec_18, mem_vec_561);
										mem_vec_561 = vec_17;

										vec_20 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f + 32]);

										vec_19 = _mm512_fmadd_ps(vec_15, vec_20, mem_vec_562);
										mem_vec_562 = vec_19;

										vec_22 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f + 48]);

										vec_21 = _mm512_fmadd_ps(vec_15, vec_22, mem_vec_563);
										mem_vec_563 = vec_21;
										scal_3 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h + 1) + c];
										vec_24 = _mm512_set1_ps(scal_3);


										vec_23 = _mm512_fmadd_ps(vec_24, vec_16, mem_vec_564);
										mem_vec_564 = vec_23;



										vec_25 = _mm512_fmadd_ps(vec_24, vec_18, mem_vec_565);
										mem_vec_565 = vec_25;



										vec_26 = _mm512_fmadd_ps(vec_24, vec_20, mem_vec_566);
										mem_vec_566 = vec_26;



										vec_27 = _mm512_fmadd_ps(vec_24, vec_22, mem_vec_567);
										mem_vec_567 = vec_27;
										scal_4 = input[(C * (Y + H - 1)) * (x + w) + C * (y + h + 2) + c];
										vec_29 = _mm512_set1_ps(scal_4);
										vec_30 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f]);

										vec_28 = _mm512_fmadd_ps(vec_29, vec_30, mem_vec_560);
										mem_vec_560 = vec_28;

										vec_32 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f + 16]);

										vec_31 = _mm512_fmadd_ps(vec_29, vec_32, mem_vec_561);
										mem_vec_561 = vec_31;

										vec_34 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f + 32]);

										vec_33 = _mm512_fmadd_ps(vec_29, vec_34, mem_vec_562);
										mem_vec_562 = vec_33;

										vec_36 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f + 48]);

										vec_35 = _mm512_fmadd_ps(vec_29, vec_36, mem_vec_563);
										mem_vec_563 = vec_35;
										scal_5 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h + 2) + c];
										vec_38 = _mm512_set1_ps(scal_5);


										vec_37 = _mm512_fmadd_ps(vec_38, vec_30, mem_vec_564);
										mem_vec_564 = vec_37;



										vec_39 = _mm512_fmadd_ps(vec_38, vec_32, mem_vec_565);
										mem_vec_565 = vec_39;



										vec_40 = _mm512_fmadd_ps(vec_38, vec_34, mem_vec_566);
										mem_vec_566 = vec_40;



										vec_41 = _mm512_fmadd_ps(vec_38, vec_36, mem_vec_567);
										mem_vec_567 = vec_41;
									}
