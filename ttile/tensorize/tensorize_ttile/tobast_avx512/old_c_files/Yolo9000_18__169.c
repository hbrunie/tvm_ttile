...
for (f387 = f388, f387_p_0 = 0;
	...
	for (c370 = c371, c370_p_0 = 0;
		...
		for (f386 = f387, f386_p_1 = f387_p_0, f386_p_0 = 0;
			f386 < f387 + 1024;
			f386 += 256, f386_p_1 += 256, f386_p_0 += 256){
				for (y = y216, yp_0 = 0;
					y < y216 + 2;
					y += 2, yp_0 += 2){
							...
							for (w = w216, wp_0 = 0;
								...
								for (x = x324, xp_1 = x324_p_0, xp_0 = 0;
									...
									x += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_2984 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f + 32]);
											mem_vec_2985 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f + 48]);
											mem_vec_2986 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
											mem_vec_2987 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16]);
											mem_vec_2988 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 32]);
											mem_vec_2989 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 48]);
										...
										_mm512_storeu_ps(&output[(F * Y) * x + F * y + f + 32], mem_vec_2984);
										_mm512_storeu_ps(&output[(F * Y) * x + F * y + f + 48], mem_vec_2985);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_2986);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16], mem_vec_2987);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 32], mem_vec_2988);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 48], mem_vec_2989);
								}
							}
						}
					}
							...
							for (w = w216, wp_0 = 0;
								...
								for (x = x324, xp_1 = x324_p_0, xp_0 = 0;
									...
									x += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_2992 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f + 32]);
											mem_vec_2993 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f + 48]);
											mem_vec_2994 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
											mem_vec_2995 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16]);
											mem_vec_2996 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 32]);
											mem_vec_2997 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 48]);
											mem_vec_2998 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
											mem_vec_2999 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f + 16]);
											mem_vec_3000 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f + 32]);
											mem_vec_3001 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f + 48]);
											// y = ph_y, x = 1, h = 3, w = 1, c = 2, f = 64
											// T (c, 2) (2 / 1)
											for (c = c370, cp_1 = c370_p_0, cp_0 = 0;
												c < c370 + 2;
												...
												vec_5 = _mm512_fmadd_ps(vec_1, vec_6, mem_vec_2992);
												mem_vec_2992 = vec_5;

												vec_8 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * h + F * c + f + 48]);

												vec_7 = _mm512_fmadd_ps(vec_1, vec_8, mem_vec_2993);
												mem_vec_2993 = vec_7;
												scal_1 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h) + c];
												vec_10 = _mm512_set1_ps(scal_1);
												...
												vec_11 = _mm512_fmadd_ps(vec_10, vec_4, mem_vec_2995);
												mem_vec_2995 = vec_11;



												vec_12 = _mm512_fmadd_ps(vec_10, vec_6, mem_vec_2996);
												mem_vec_2996 = vec_12;



												vec_13 = _mm512_fmadd_ps(vec_10, vec_8, mem_vec_2997);
												mem_vec_2997 = vec_13;
												...
												vec_17 = _mm512_fmadd_ps(vec_15, vec_6, mem_vec_3000);
												mem_vec_3000 = vec_17;



												vec_18 = _mm512_fmadd_ps(vec_15, vec_8, mem_vec_3001);
												mem_vec_3001 = vec_18;
												scal_3 = input[(C * (Y + H - 1)) * (x + w) + C * (y + h + 1) + c];
												vec_20 = _mm512_set1_ps(scal_3);
												vec_21 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f]);

												vec_19 = _mm512_fmadd_ps(vec_20, vec_21, mem_vec_2990);
												mem_vec_2990 = vec_19;

												vec_23 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f + 16]);

												vec_22 = _mm512_fmadd_ps(vec_20, vec_23, mem_vec_2991);
												mem_vec_2991 = vec_22;

												vec_25 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f + 32]);

												vec_24 = _mm512_fmadd_ps(vec_20, vec_25, mem_vec_2992);
												mem_vec_2992 = vec_24;

												vec_27 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f + 48]);

												vec_26 = _mm512_fmadd_ps(vec_20, vec_27, mem_vec_2993);
												mem_vec_2993 = vec_26;
												scal_4 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h + 1) + c];
												vec_29 = _mm512_set1_ps(scal_4);


												vec_28 = _mm512_fmadd_ps(vec_29, vec_21, mem_vec_2994);
												mem_vec_2994 = vec_28;



												vec_30 = _mm512_fmadd_ps(vec_29, vec_23, mem_vec_2995);
												mem_vec_2995 = vec_30;



												vec_31 = _mm512_fmadd_ps(vec_29, vec_25, mem_vec_2996);
												mem_vec_2996 = vec_31;



												vec_32 = _mm512_fmadd_ps(vec_29, vec_27, mem_vec_2997);
												mem_vec_2997 = vec_32;
												scal_5 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 2 + h + 1) + c];
												vec_34 = _mm512_set1_ps(scal_5);


												vec_33 = _mm512_fmadd_ps(vec_34, vec_21, mem_vec_2998);
												mem_vec_2998 = vec_33;



												vec_35 = _mm512_fmadd_ps(vec_34, vec_23, mem_vec_2999);
												mem_vec_2999 = vec_35;



												vec_36 = _mm512_fmadd_ps(vec_34, vec_25, mem_vec_3000);
												mem_vec_3000 = vec_36;



												vec_37 = _mm512_fmadd_ps(vec_34, vec_27, mem_vec_3001);
												mem_vec_3001 = vec_37;
												scal_6 = input[(C * (Y + H - 1)) * (x + w) + C * (y + h + 2) + c];
												vec_39 = _mm512_set1_ps(scal_6);
												vec_40 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f]);

												vec_38 = _mm512_fmadd_ps(vec_39, vec_40, mem_vec_2990);
												mem_vec_2990 = vec_38;

												vec_42 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f + 16]);

												vec_41 = _mm512_fmadd_ps(vec_39, vec_42, mem_vec_2991);
												mem_vec_2991 = vec_41;

												vec_44 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f + 32]);

												vec_43 = _mm512_fmadd_ps(vec_39, vec_44, mem_vec_2992);
												mem_vec_2992 = vec_43;

												vec_46 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f + 48]);

												vec_45 = _mm512_fmadd_ps(vec_39, vec_46, mem_vec_2993);
												mem_vec_2993 = vec_45;
												scal_7 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h + 2) + c];
												vec_48 = _mm512_set1_ps(scal_7);


												vec_47 = _mm512_fmadd_ps(vec_48, vec_40, mem_vec_2994);
												mem_vec_2994 = vec_47;



												vec_49 = _mm512_fmadd_ps(vec_48, vec_42, mem_vec_2995);
												mem_vec_2995 = vec_49;



												vec_50 = _mm512_fmadd_ps(vec_48, vec_44, mem_vec_2996);
												mem_vec_2996 = vec_50;



												vec_51 = _mm512_fmadd_ps(vec_48, vec_46, mem_vec_2997);
												mem_vec_2997 = vec_51;
												scal_8 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 2 + h + 2) + c];
												vec_53 = _mm512_set1_ps(scal_8);


												vec_52 = _mm512_fmadd_ps(vec_53, vec_40, mem_vec_2998);
												mem_vec_2998 = vec_52;



												vec_54 = _mm512_fmadd_ps(vec_53, vec_42, mem_vec_2999);
												mem_vec_2999 = vec_54;



												vec_55 = _mm512_fmadd_ps(vec_53, vec_44, mem_vec_3000);
												mem_vec_3000 = vec_55;



												vec_56 = _mm512_fmadd_ps(vec_53, vec_46, mem_vec_3001);
												mem_vec_3001 = vec_56;
											}
										_mm512_storeu_ps(&output[(F * Y) * x + F * y + f], mem_vec_2990);
										_mm512_storeu_ps(&output[(F * Y) * x + F * y + f + 16], mem_vec_2991);
										_mm512_storeu_ps(&output[(F * Y) * x + F * y + f + 32], mem_vec_2992);
										_mm512_storeu_ps(&output[(F * Y) * x + F * y + f + 48], mem_vec_2993);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_2994);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16], mem_vec_2995);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 32], mem_vec_2996);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 48], mem_vec_2997);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_2998);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f + 16], mem_vec_2999);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f + 32], mem_vec_3000);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f + 48], mem_vec_3001);
								}
							}
						}
					}
