					...
					for (x280 = x281, x280_p_1 = x281_p_0, x280_p_0 = 0;
						...
						for (x = x280, xp_2 = x280_p_1, xp_1 = x280_p_0, xp_0 = 0;
							...
							x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
									...
									mem_vec_2502 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
									mem_vec_2503 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
									// y = ph_y, x = 1, h = 1, w = 1, c = 128, f = 32
									// T (c, 128) (128 / 1)
									for (c = c210, cp_1 = c210_p_0, cp_0 = 0;
										c < c210 + 128;
								...
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_2502);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_2503);
						}
					}
				}
		}
