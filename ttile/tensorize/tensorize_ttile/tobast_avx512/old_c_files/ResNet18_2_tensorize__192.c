...
x202_p_1 = 0;
					for (h = h74, hp_0 = 0;h < h74 + 3;h += 1, hp_0 += 1){
						...
						// T (x, 7) (7 / 1)
						for (x = x202, xp_2 = x202_p_1, xp_1 = x202_p_0, xp_0 = 0;x < x202 + 7;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
									mem_vec_842 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * y + f]);
									mem_vec_843 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f]);
									mem_vec_844 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f]);
									mem_vec_845 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f]);
									// y = 4, x = 1, h = 1, w = 3, c = 16, f = 16
									// T (c, 16) (16 / 1)
									for (c = c176, cp_1 = c176_p_0, cp_0 = 0;c < c176 + 16;c += 1, cp_1 += 1, cp_0 += 1){
								...
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f], mem_vec_843);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f], mem_vec_844);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f], mem_vec_845);
						}
					}
}
