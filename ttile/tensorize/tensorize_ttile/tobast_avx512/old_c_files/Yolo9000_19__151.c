					...
					for (c128 = c129, c128_p_1 = c129_p_0, c128_p_0 = 0;
						...
						for (x = x96, xp_1 = x96_p_0, xp_0 = 0;
							...
							x += 1, xp_1 += 1, xp_0 += 1){
									...
									mem_vec_1120 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f]);
									mem_vec_1121 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16]);
									// y = ph_y, x = 1, h = 1, w = 1, c = 16, f = 32
									// T (c, 16) (16 / 1)
									for (c = c128, cp_2 = c128_p_1, cp_1 = c128_p_0, cp_0 = 0;
										c < c128 + 16;
								...
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f], mem_vec_1120);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16], mem_vec_1121);
						}
					}
				}
			}
