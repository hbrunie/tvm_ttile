#include "../gen_conv.h"
#include "../reorder_avx2.h"

void gen_conv(M_TYPE * const  __restrict__ output,
M_TYPE const * const __restrict__ input, M_TYPE const * const __restrict__ params,
    IND_TYPE X, IND_TYPE W,
    IND_TYPE Y, IND_TYPE H,
	IND_TYPE C, IND_TYPE F) {
/*
[V f; U (2, f); ULambda y; T (4, c); Hoist_vars [c]; T (34, x); T (3, w);
  T (3, h); T (2, f); T (1, x);
  Lambda_apply y [((Iter 1), (Arg 8)); ((Iter 2), (Arg 13))]; T (1, f);
  T (64, c); T (8, f)]
*/
IND_TYPE c, cp_0, c66_p_0, cp_1, c66, f, fp_0, f88_p_0, f89_p_0, fp_1, f88_p_1, fp_2, f88, f89, h, hp_0, w, wp_0, x, xp_0, x66_p_0, xp_1, x66, y, yp_0;

assert((Y == 34));
assert((X == 34));
assert((H == 3));
assert((W == 3));
assert((C == 256));
assert((F == 512));
IND_TYPE y44 = 0;
IND_TYPE x67 = 0;
IND_TYPE h36 = 0;
IND_TYPE w42 = 0;
IND_TYPE c67 = 0;
IND_TYPE f90 = 0;
float scal_0 ,scal_1 ,scal_10 ,scal_11 ,scal_12 ,scal_2 ,scal_3 ,scal_4 ,scal_5 ,scal_6 ,scal_7 ,scal_8 ,scal_9;
__m512 mem_vec_492 ,mem_vec_493 ,mem_vec_494 ,mem_vec_495 ,mem_vec_496 ,mem_vec_497 ,mem_vec_498 ,mem_vec_499 ,mem_vec_500 ,mem_vec_501 ,mem_vec_502 ,mem_vec_503 ,mem_vec_504 ,mem_vec_505 ,mem_vec_506 ,mem_vec_507 ,mem_vec_508 ,mem_vec_509 ,mem_vec_510 ,mem_vec_511 ,mem_vec_512 ,mem_vec_513 ,mem_vec_514 ,mem_vec_515 ,mem_vec_516 ,mem_vec_517 ,mem_vec_518 ,mem_vec_519 ,mem_vec_520 ,mem_vec_521 ,mem_vec_522 ,mem_vec_523 ,mem_vec_524 ,mem_vec_525 ,mem_vec_526 ,mem_vec_527 ,mem_vec_528 ,mem_vec_529 ,mem_vec_530 ,mem_vec_531 ,mem_vec_532 ,mem_vec_533 ,vec_0 ,vec_1 ,vec_10 ,vec_11 ,vec_12 ,vec_13 ,vec_14 ,vec_15 ,vec_16 ,vec_17 ,vec_18 ,vec_19 ,vec_2 ,vec_20 ,vec_21 ,vec_22 ,vec_23 ,vec_24 ,vec_25 ,vec_26 ,vec_27 ,vec_28 ,vec_29 ,vec_3 ,vec_30 ,vec_31 ,vec_32 ,vec_33 ,vec_34 ,vec_35 ,vec_36 ,vec_37 ,vec_38 ,vec_39 ,vec_4 ,vec_40 ,vec_5 ,vec_6 ,vec_7 ,vec_8 ,vec_9;
// y = 34, x = 34, h = 3, w = 3, c = 256, f = 512
// T (f, 8) (512 / 64)
for (f89 = f90, f89_p_0 = 0;
	f89 < f90 + 512;
	f89 += 64, f89_p_0 += 64){
	// y = 34, x = 34, h = 3, w = 3, c = 256, f = 64
	// T (c, 64) (256 / 4)
	for (c66 = c67, c66_p_0 = 0;
		c66 < c67 + 256;
		c66 += 4, c66_p_0 += 4){
		// y = 34, x = 34, h = 3, w = 3, c = 4, f = 64
		// T (f, 1) (64 / 64)
		for (f88 = f89, f88_p_1 = f89_p_0, f88_p_0 = 0;
			f88 < f89 + 64;
			f88 += 64, f88_p_1 += 64, f88_p_0 += 64){
				for (y = y44, yp_0 = 0;
					y < y44 + 8;
					y += 8, yp_0 += 8){
					// y = ph_y, x = 34, h = 3, w = 3, c = 4, f = 64
					// T (x, 1) (34 / 34)
					for (x66 = x67, x66_p_0 = 0;
						x66 < x67 + 34;
						x66 += 34, x66_p_0 += 34){
						// y = ph_y, x = 34, h = 3, w = 3, c = 4, f = 64
						// T (f, 2) (64 / 32)
						for (f = f88, fp_2 = f88_p_1, fp_1 = f88_p_0, fp_0 = 0;
							f < f88 + 64;
							f += 32, fp_2 += 32, fp_1 += 32, fp_0 += 32){
							// y = ph_y, x = 34, h = 3, w = 3, c = 4, f = 32
							// T (h, 3) (3 / 1)
							for (h = h36, hp_0 = 0;
								h < h36 + 3;
								h += 1, hp_0 += 1){
								// y = ph_y, x = 34, h = 1, w = 3, c = 4, f = 32
								// T (w, 3) (3 / 1)
								for (w = w42, wp_0 = 0;
									w < w42 + 3;
									w += 1, wp_0 += 1){
									// y = ph_y, x = 34, h = 1, w = 1, c = 4, f = 32
									// T (x, 34) (34 / 1)
									for (x = x66, xp_1 = x66_p_0, xp_0 = 0;
										x < x66 + 34;
										x += 1, xp_1 += 1, xp_0 += 1){
												mem_vec_492 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f]);
												mem_vec_493 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f + 16]);
												mem_vec_494 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
												mem_vec_495 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16]);
												mem_vec_496 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
												mem_vec_497 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f + 16]);
												mem_vec_498 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
												mem_vec_499 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f + 16]);
												mem_vec_500 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
												mem_vec_501 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16]);
												mem_vec_502 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
												mem_vec_503 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f + 16]);
												mem_vec_504 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
												mem_vec_505 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 16]);
												mem_vec_506 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 7) + f]);
												mem_vec_507 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 7) + f + 16]);
												// y = ph_y, x = 1, h = 1, w = 1, c = 4, f = 32
												// T (c, 4) (4 / 1)
												for (c = c66, cp_1 = c66_p_0, cp_0 = 0;
													c < c66 + 4;
													c += 1, cp_1 += 1, cp_0 += 1){
													scal_0 = input[(C * (Y + H - 1)) * (x + w) + C * (y + h) + c];
													vec_1 = _mm512_set1_ps(scal_0);
													vec_2 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * h + F * c + f]);

													vec_0 = _mm512_fmadd_ps(vec_1, vec_2, mem_vec_492);
													mem_vec_492 = vec_0;

													vec_4 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * h + F * c + f + 16]);

													vec_3 = _mm512_fmadd_ps(vec_1, vec_4, mem_vec_493);
													mem_vec_493 = vec_3;
													scal_1 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h) + c];
													vec_6 = _mm512_set1_ps(scal_1);


													vec_5 = _mm512_fmadd_ps(vec_6, vec_2, mem_vec_494);
													mem_vec_494 = vec_5;



													vec_7 = _mm512_fmadd_ps(vec_6, vec_4, mem_vec_495);
													mem_vec_495 = vec_7;
													scal_2 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 2 + h) + c];
													vec_9 = _mm512_set1_ps(scal_2);


													vec_8 = _mm512_fmadd_ps(vec_9, vec_2, mem_vec_496);
													mem_vec_496 = vec_8;



													vec_10 = _mm512_fmadd_ps(vec_9, vec_4, mem_vec_497);
													mem_vec_497 = vec_10;
													scal_3 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 3 + h) + c];
													vec_12 = _mm512_set1_ps(scal_3);


													vec_11 = _mm512_fmadd_ps(vec_12, vec_2, mem_vec_498);
													mem_vec_498 = vec_11;



													vec_13 = _mm512_fmadd_ps(vec_12, vec_4, mem_vec_499);
													mem_vec_499 = vec_13;
													scal_4 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 4 + h) + c];
													vec_15 = _mm512_set1_ps(scal_4);


													vec_14 = _mm512_fmadd_ps(vec_15, vec_2, mem_vec_500);
													mem_vec_500 = vec_14;



													vec_16 = _mm512_fmadd_ps(vec_15, vec_4, mem_vec_501);
													mem_vec_501 = vec_16;
													scal_5 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 5 + h) + c];
													vec_18 = _mm512_set1_ps(scal_5);


													vec_17 = _mm512_fmadd_ps(vec_18, vec_2, mem_vec_502);
													mem_vec_502 = vec_17;



													vec_19 = _mm512_fmadd_ps(vec_18, vec_4, mem_vec_503);
													mem_vec_503 = vec_19;
													scal_6 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 6 + h) + c];
													vec_21 = _mm512_set1_ps(scal_6);


													vec_20 = _mm512_fmadd_ps(vec_21, vec_2, mem_vec_504);
													mem_vec_504 = vec_20;



													vec_22 = _mm512_fmadd_ps(vec_21, vec_4, mem_vec_505);
													mem_vec_505 = vec_22;
													scal_7 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 7 + h) + c];
													vec_24 = _mm512_set1_ps(scal_7);


													vec_23 = _mm512_fmadd_ps(vec_24, vec_2, mem_vec_506);
													mem_vec_506 = vec_23;



													vec_25 = _mm512_fmadd_ps(vec_24, vec_4, mem_vec_507);
													mem_vec_507 = vec_25;
												}
											_mm512_storeu_ps(&output[(F * Y) * x + F * y + f], mem_vec_492);
											_mm512_storeu_ps(&output[(F * Y) * x + F * y + f + 16], mem_vec_493);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_494);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16], mem_vec_495);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_496);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f + 16], mem_vec_497);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f], mem_vec_498);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f + 16], mem_vec_499);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_500);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16], mem_vec_501);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f], mem_vec_502);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f + 16], mem_vec_503);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f], mem_vec_504);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 16], mem_vec_505);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 7) + f], mem_vec_506);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 7) + f + 16], mem_vec_507);
									}
								}
							}
						}
					}
				}
				for (y = y44 + 8, yp_0 = 0;
					y < y44 + 8 + 26;
					y += 13, yp_0 += 13){
					// y = ph_y, x = 34, h = 3, w = 3, c = 4, f = 64
					// T (x, 1) (34 / 34)
					for (x66 = x67, x66_p_0 = 0;
						x66 < x67 + 34;
						x66 += 34, x66_p_0 += 34){
						// y = ph_y, x = 34, h = 3, w = 3, c = 4, f = 64
						// T (f, 2) (64 / 32)
						for (f = f88, fp_2 = f88_p_1, fp_1 = f88_p_0, fp_0 = 0;
							f < f88 + 64;
							f += 32, fp_2 += 32, fp_1 += 32, fp_0 += 32){
							// y = ph_y, x = 34, h = 3, w = 3, c = 4, f = 32
							// T (h, 3) (3 / 1)
							for (h = h36, hp_0 = 0;
								h < h36 + 3;
								h += 1, hp_0 += 1){
								// y = ph_y, x = 34, h = 1, w = 3, c = 4, f = 32
								// T (w, 3) (3 / 1)
								for (w = w42, wp_0 = 0;
									w < w42 + 3;
									w += 1, wp_0 += 1){
									// y = ph_y, x = 34, h = 1, w = 1, c = 4, f = 32
									// T (x, 34) (34 / 1)
									for (x = x66, xp_1 = x66_p_0, xp_0 = 0;
										x < x66 + 34;
										x += 1, xp_1 += 1, xp_0 += 1){
												mem_vec_508 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f]);
												mem_vec_509 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f + 16]);
												mem_vec_510 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
												mem_vec_511 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16]);
												mem_vec_512 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
												mem_vec_513 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f + 16]);
												mem_vec_514 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
												mem_vec_515 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f + 16]);
												mem_vec_516 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
												mem_vec_517 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16]);
												mem_vec_518 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
												mem_vec_519 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f + 16]);
												mem_vec_520 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
												mem_vec_521 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 16]);
												mem_vec_522 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 7) + f]);
												mem_vec_523 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 7) + f + 16]);
												mem_vec_524 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 8) + f]);
												mem_vec_525 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 8) + f + 16]);
												mem_vec_526 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f]);
												mem_vec_527 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16]);
												mem_vec_528 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f]);
												mem_vec_529 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16]);
												mem_vec_530 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f]);
												mem_vec_531 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16]);
												mem_vec_532 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
												mem_vec_533 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
												// y = ph_y, x = 1, h = 1, w = 1, c = 4, f = 32
												// T (c, 4) (4 / 1)
												for (c = c66, cp_1 = c66_p_0, cp_0 = 0;
													c < c66 + 4;
													c += 1, cp_1 += 1, cp_0 += 1){
													scal_0 = input[(C * (Y + H - 1)) * (x + w) + C * (y + h) + c];
													vec_1 = _mm512_set1_ps(scal_0);
													vec_2 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * h + F * c + f]);

													vec_0 = _mm512_fmadd_ps(vec_1, vec_2, mem_vec_508);
													mem_vec_508 = vec_0;

													vec_4 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * h + F * c + f + 16]);

													vec_3 = _mm512_fmadd_ps(vec_1, vec_4, mem_vec_509);
													mem_vec_509 = vec_3;
													scal_1 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h) + c];
													vec_6 = _mm512_set1_ps(scal_1);


													vec_5 = _mm512_fmadd_ps(vec_6, vec_2, mem_vec_510);
													mem_vec_510 = vec_5;



													vec_7 = _mm512_fmadd_ps(vec_6, vec_4, mem_vec_511);
													mem_vec_511 = vec_7;
													scal_2 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 2 + h) + c];
													vec_9 = _mm512_set1_ps(scal_2);


													vec_8 = _mm512_fmadd_ps(vec_9, vec_2, mem_vec_512);
													mem_vec_512 = vec_8;



													vec_10 = _mm512_fmadd_ps(vec_9, vec_4, mem_vec_513);
													mem_vec_513 = vec_10;
													scal_3 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 3 + h) + c];
													vec_12 = _mm512_set1_ps(scal_3);


													vec_11 = _mm512_fmadd_ps(vec_12, vec_2, mem_vec_514);
													mem_vec_514 = vec_11;



													vec_13 = _mm512_fmadd_ps(vec_12, vec_4, mem_vec_515);
													mem_vec_515 = vec_13;
													scal_4 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 4 + h) + c];
													vec_15 = _mm512_set1_ps(scal_4);


													vec_14 = _mm512_fmadd_ps(vec_15, vec_2, mem_vec_516);
													mem_vec_516 = vec_14;



													vec_16 = _mm512_fmadd_ps(vec_15, vec_4, mem_vec_517);
													mem_vec_517 = vec_16;
													scal_5 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 5 + h) + c];
													vec_18 = _mm512_set1_ps(scal_5);


													vec_17 = _mm512_fmadd_ps(vec_18, vec_2, mem_vec_518);
													mem_vec_518 = vec_17;



													vec_19 = _mm512_fmadd_ps(vec_18, vec_4, mem_vec_519);
													mem_vec_519 = vec_19;
													scal_6 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 6 + h) + c];
													vec_21 = _mm512_set1_ps(scal_6);


													vec_20 = _mm512_fmadd_ps(vec_21, vec_2, mem_vec_520);
													mem_vec_520 = vec_20;



													vec_22 = _mm512_fmadd_ps(vec_21, vec_4, mem_vec_521);
													mem_vec_521 = vec_22;
													scal_7 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 7 + h) + c];
													vec_24 = _mm512_set1_ps(scal_7);


													vec_23 = _mm512_fmadd_ps(vec_24, vec_2, mem_vec_522);
													mem_vec_522 = vec_23;



													vec_25 = _mm512_fmadd_ps(vec_24, vec_4, mem_vec_523);
													mem_vec_523 = vec_25;
													scal_8 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 8 + h) + c];
													vec_27 = _mm512_set1_ps(scal_8);


													vec_26 = _mm512_fmadd_ps(vec_27, vec_2, mem_vec_524);
													mem_vec_524 = vec_26;



													vec_28 = _mm512_fmadd_ps(vec_27, vec_4, mem_vec_525);
													mem_vec_525 = vec_28;
													scal_9 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 9 + h) + c];
													vec_30 = _mm512_set1_ps(scal_9);


													vec_29 = _mm512_fmadd_ps(vec_30, vec_2, mem_vec_526);
													mem_vec_526 = vec_29;



													vec_31 = _mm512_fmadd_ps(vec_30, vec_4, mem_vec_527);
													mem_vec_527 = vec_31;
													scal_10 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 10 + h) + c];
													vec_33 = _mm512_set1_ps(scal_10);


													vec_32 = _mm512_fmadd_ps(vec_33, vec_2, mem_vec_528);
													mem_vec_528 = vec_32;



													vec_34 = _mm512_fmadd_ps(vec_33, vec_4, mem_vec_529);
													mem_vec_529 = vec_34;
													scal_11 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 11 + h) + c];
													vec_36 = _mm512_set1_ps(scal_11);


													vec_35 = _mm512_fmadd_ps(vec_36, vec_2, mem_vec_530);
													mem_vec_530 = vec_35;



													vec_37 = _mm512_fmadd_ps(vec_36, vec_4, mem_vec_531);
													mem_vec_531 = vec_37;
													scal_12 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 12 + h) + c];
													vec_39 = _mm512_set1_ps(scal_12);


													vec_38 = _mm512_fmadd_ps(vec_39, vec_2, mem_vec_532);
													mem_vec_532 = vec_38;



													vec_40 = _mm512_fmadd_ps(vec_39, vec_4, mem_vec_533);
													mem_vec_533 = vec_40;
												}
											_mm512_storeu_ps(&output[(F * Y) * x + F * y + f], mem_vec_508);
											_mm512_storeu_ps(&output[(F * Y) * x + F * y + f + 16], mem_vec_509);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_510);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16], mem_vec_511);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_512);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f + 16], mem_vec_513);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f], mem_vec_514);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f + 16], mem_vec_515);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_516);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16], mem_vec_517);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f], mem_vec_518);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f + 16], mem_vec_519);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f], mem_vec_520);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 16], mem_vec_521);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 7) + f], mem_vec_522);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 7) + f + 16], mem_vec_523);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 8) + f], mem_vec_524);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 8) + f + 16], mem_vec_525);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f], mem_vec_526);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16], mem_vec_527);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f], mem_vec_528);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16], mem_vec_529);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f], mem_vec_530);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16], mem_vec_531);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_532);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_533);
									}
								}
							}
						}
					}
				}
		}
	}
}


}