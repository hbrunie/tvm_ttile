...
for (f113 = f114, f113_p_0 = 0;
	...
	for (c84 = c85, c84_p_0 = 0;
		...
		for (f112 = f113, f112_p_1 = f113_p_0, f112_p_0 = 0;
			f112 < f113 + 512;
			f112 += 512, f112_p_1 += 512, f112_p_0 += 512){
				for (y = y56, yp_0 = 0;
					y < y56 + 20;
					y += 10, yp_0 += 10){
								...
								for (w = w54, wp_0 = 0;
									...
									for (x = x84, xp_1 = x84_p_0, xp_0 = 0;
										...
										x += 1, xp_1 += 1, xp_0 += 1){
												...
												mem_vec_794 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f]);
												mem_vec_795 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16]);
												// y = ph_y, x = 1, h = 1, w = 1, c = 128, f = 32
												// T (c, 128) (128 / 1)
												for (c = c84, cp_1 = c84_p_0, cp_0 = 0;
													c < c84 + 128;
											...
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f], mem_vec_794);
											_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16], mem_vec_795);
									}
								}
							}
						}
