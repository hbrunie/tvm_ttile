					...
					for (h = h84, hp_0 = 0;h < h84 + 3;h += 1, hp_0 += 1){
						...
						for (w = w94, wp_0 = 0;w < w94 + 3;w += 1, wp_0 += 1){
							...
							for (x = x256, xp_2 = x256_p_1, xp_1 = x256_p_0, xp_0 = 0;x < x256 + 1;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_950 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f]);
										mem_vec_951 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f + 16]);
										// y = ph_y, x = 1, h = 1, w = 1, c = 2, f = 32
										// T (c, 2) (2 / 1)
									...
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f], mem_vec_950);
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f + 16], mem_vec_951);
							}
						}
					}
}
