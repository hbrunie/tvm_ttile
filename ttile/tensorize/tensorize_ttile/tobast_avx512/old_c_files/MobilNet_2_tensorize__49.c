...
/*
[V f; U (1, f); U (7, y); U (3, h); T (32, c); Hoist_vars [c]; T (1, x);
  T (3, w); T (1, x); T (8, y); T (4, f); T (2, c); T (56, x)]
*/
IND_TYPE c, cp_0, c68_p_0, cp_1, c68, f, fp_0, w, wp_0, x, xp_0, x72_p_0, x73_p_0, xp_1, x72_p_1, xp_2, x72, x73, y, yp_0;
IND_TYPE y40 = 0;
IND_TYPE x74 = 0;
IND_TYPE h = 0;
IND_TYPE w40 = 0;
...
x72_p_1 = 0;
					for (w = w40, wp_0 = 0;w < w40 + 3;w += 1, wp_0 += 1){
						...
						for (x = x72, xp_2 = x72_p_1, xp_1 = x72_p_0, xp_0 = 0;x < x72 + 1;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
									...
									mem_vec_325 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f]);
									mem_vec_326 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f]);
									mem_vec_327 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f]);
									mem_vec_328 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f]);
									mem_vec_329 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f]);
									mem_vec_330 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 6) + f]);
									// y = 7, x = 1, h = 3, w = 1, c = 32, f = 16
									// T (c, 32) (32 / 1)
									for (c = c68, cp_1 = c68_p_0, cp_0 = 0;c < c68 + 32;c += 1, cp_1 += 1, cp_0 += 1){
								...
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f], mem_vec_325);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f], mem_vec_326);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f], mem_vec_327);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f], mem_vec_328);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f], mem_vec_329);
