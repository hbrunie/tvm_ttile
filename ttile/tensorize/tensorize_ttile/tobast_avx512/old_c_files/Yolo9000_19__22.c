#include "../gen_conv.h"
#include "../reorder_avx2.h"

void gen_conv(M_TYPE * const  __restrict__ output,
M_TYPE const * const __restrict__ input, M_TYPE const * const __restrict__ params,
    IND_TYPE X, IND_TYPE W,
    IND_TYPE Y, IND_TYPE H,
	IND_TYPE C, IND_TYPE F) {
/*
[V f; U (2, f); ULambda y; T (4, c); Hoist_vars [c]; T (17, x); T (2, c);
  T (16, f); T (1, x);
  Lambda_apply y [((Iter 1), (Arg 7)); ((Iter 1), (Arg 10))]; T (128, c)]
*/
IND_TYPE c, cp_0, c96_p_0, c97_p_0, cp_1, c96_p_1, cp_2, c96, c97, f, fp_0, x, xp_0, x72_p_0, xp_1, x72, y, yp_0;

assert((Y == 17));
assert((X == 17));
assert((H == 1));
assert((W == 1));
assert((C == 1024));
assert((F == 512));
IND_TYPE y48 = 0;
IND_TYPE x73 = 0;
IND_TYPE h = 0;
IND_TYPE w = 0;
IND_TYPE c98 = 0;
IND_TYPE f48 = 0;
float scal_0 ,scal_1 ,scal_2 ,scal_3 ,scal_4 ,scal_5 ,scal_6 ,scal_7 ,scal_8 ,scal_9;
__m512 mem_vec_816 ,mem_vec_817 ,mem_vec_818 ,mem_vec_819 ,mem_vec_820 ,mem_vec_821 ,mem_vec_822 ,mem_vec_823 ,mem_vec_824 ,mem_vec_825 ,mem_vec_826 ,mem_vec_827 ,mem_vec_828 ,mem_vec_829 ,mem_vec_830 ,mem_vec_831 ,mem_vec_832 ,mem_vec_833 ,mem_vec_834 ,mem_vec_835 ,mem_vec_836 ,mem_vec_837 ,mem_vec_838 ,mem_vec_839 ,mem_vec_840 ,mem_vec_841 ,mem_vec_842 ,mem_vec_843 ,mem_vec_844 ,mem_vec_845 ,mem_vec_846 ,mem_vec_847 ,mem_vec_848 ,mem_vec_849 ,vec_0 ,vec_1 ,vec_10 ,vec_11 ,vec_12 ,vec_13 ,vec_14 ,vec_15 ,vec_16 ,vec_17 ,vec_18 ,vec_19 ,vec_2 ,vec_20 ,vec_21 ,vec_22 ,vec_23 ,vec_24 ,vec_25 ,vec_26 ,vec_27 ,vec_28 ,vec_29 ,vec_3 ,vec_30 ,vec_31 ,vec_4 ,vec_5 ,vec_6 ,vec_7 ,vec_8 ,vec_9;
// y = 17, x = 17, h = 1, w = 1, c = 1024, f = 512
// T (c, 128) (1024 / 8)
for (c97 = c98, c97_p_0 = 0;
	c97 < c98 + 1024;
	c97 += 8, c97_p_0 += 8){
		for (y = y48, yp_0 = 0;
			y < y48 + 7;
			y += 7, yp_0 += 7){
			// y = ph_y, x = 17, h = 1, w = 1, c = 8, f = 512
			// T (x, 1) (17 / 17)
			for (x72 = x73, x72_p_0 = 0;
				x72 < x73 + 17;
				x72 += 17, x72_p_0 += 17){
				// y = ph_y, x = 17, h = 1, w = 1, c = 8, f = 512
				// T (f, 16) (512 / 32)
				for (f = f48, fp_0 = 0;
					f < f48 + 512;
					f += 32, fp_0 += 32){
					// y = ph_y, x = 17, h = 1, w = 1, c = 8, f = 32
					// T (c, 2) (8 / 4)
					for (c96 = c97, c96_p_1 = c97_p_0, c96_p_0 = 0;
						c96 < c97 + 8;
						c96 += 4, c96_p_1 += 4, c96_p_0 += 4){
						// y = ph_y, x = 17, h = 1, w = 1, c = 4, f = 32
						// T (x, 17) (17 / 1)
						for (x = x72, xp_1 = x72_p_0, xp_0 = 0;
							x < x72 + 17;
							x += 1, xp_1 += 1, xp_0 += 1){
									mem_vec_816 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f]);
									mem_vec_817 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f + 16]);
									mem_vec_818 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
									mem_vec_819 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16]);
									mem_vec_820 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
									mem_vec_821 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f + 16]);
									mem_vec_822 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
									mem_vec_823 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f + 16]);
									mem_vec_824 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
									mem_vec_825 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16]);
									mem_vec_826 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
									mem_vec_827 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f + 16]);
									mem_vec_828 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
									mem_vec_829 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 16]);
									// y = ph_y, x = 1, h = 1, w = 1, c = 4, f = 32
									// T (c, 4) (4 / 1)
									for (c = c96, cp_2 = c96_p_1, cp_1 = c96_p_0, cp_0 = 0;
										c < c96 + 4;
										c += 1, cp_2 += 1, cp_1 += 1, cp_0 += 1){
										scal_0 = input[(C * (Y + H - 1)) * (x + w) + C * (y + h) + c];
										vec_1 = _mm512_set1_ps(scal_0);
										vec_2 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * h + F * c + f]);

										vec_0 = _mm512_fmadd_ps(vec_1, vec_2, mem_vec_816);
										mem_vec_816 = vec_0;

										vec_4 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * h + F * c + f + 16]);

										vec_3 = _mm512_fmadd_ps(vec_1, vec_4, mem_vec_817);
										mem_vec_817 = vec_3;
										scal_1 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h) + c];
										vec_6 = _mm512_set1_ps(scal_1);


										vec_5 = _mm512_fmadd_ps(vec_6, vec_2, mem_vec_818);
										mem_vec_818 = vec_5;



										vec_7 = _mm512_fmadd_ps(vec_6, vec_4, mem_vec_819);
										mem_vec_819 = vec_7;
										scal_2 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 2 + h) + c];
										vec_9 = _mm512_set1_ps(scal_2);


										vec_8 = _mm512_fmadd_ps(vec_9, vec_2, mem_vec_820);
										mem_vec_820 = vec_8;



										vec_10 = _mm512_fmadd_ps(vec_9, vec_4, mem_vec_821);
										mem_vec_821 = vec_10;
										scal_3 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 3 + h) + c];
										vec_12 = _mm512_set1_ps(scal_3);


										vec_11 = _mm512_fmadd_ps(vec_12, vec_2, mem_vec_822);
										mem_vec_822 = vec_11;



										vec_13 = _mm512_fmadd_ps(vec_12, vec_4, mem_vec_823);
										mem_vec_823 = vec_13;
										scal_4 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 4 + h) + c];
										vec_15 = _mm512_set1_ps(scal_4);


										vec_14 = _mm512_fmadd_ps(vec_15, vec_2, mem_vec_824);
										mem_vec_824 = vec_14;



										vec_16 = _mm512_fmadd_ps(vec_15, vec_4, mem_vec_825);
										mem_vec_825 = vec_16;
										scal_5 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 5 + h) + c];
										vec_18 = _mm512_set1_ps(scal_5);


										vec_17 = _mm512_fmadd_ps(vec_18, vec_2, mem_vec_826);
										mem_vec_826 = vec_17;



										vec_19 = _mm512_fmadd_ps(vec_18, vec_4, mem_vec_827);
										mem_vec_827 = vec_19;
										scal_6 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 6 + h) + c];
										vec_21 = _mm512_set1_ps(scal_6);


										vec_20 = _mm512_fmadd_ps(vec_21, vec_2, mem_vec_828);
										mem_vec_828 = vec_20;



										vec_22 = _mm512_fmadd_ps(vec_21, vec_4, mem_vec_829);
										mem_vec_829 = vec_22;
									}
								_mm512_storeu_ps(&output[(F * Y) * x + F * y + f], mem_vec_816);
								_mm512_storeu_ps(&output[(F * Y) * x + F * y + f + 16], mem_vec_817);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_818);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16], mem_vec_819);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_820);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f + 16], mem_vec_821);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f], mem_vec_822);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f + 16], mem_vec_823);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_824);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16], mem_vec_825);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f], mem_vec_826);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f + 16], mem_vec_827);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f], mem_vec_828);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 16], mem_vec_829);
						}
					}
				}
			}
		}
		for (y = y48 + 7, yp_0 = 0;
			y < y48 + 7 + 10;
			y += 10, yp_0 += 10){
			// y = ph_y, x = 17, h = 1, w = 1, c = 8, f = 512
			// T (x, 1) (17 / 17)
			for (x72 = x73, x72_p_0 = 0;
				x72 < x73 + 17;
				x72 += 17, x72_p_0 += 17){
				// y = ph_y, x = 17, h = 1, w = 1, c = 8, f = 512
				// T (f, 16) (512 / 32)
				for (f = f48, fp_0 = 0;
					f < f48 + 512;
					f += 32, fp_0 += 32){
					// y = ph_y, x = 17, h = 1, w = 1, c = 8, f = 32
					// T (c, 2) (8 / 4)
					for (c96 = c97, c96_p_1 = c97_p_0, c96_p_0 = 0;
						c96 < c97 + 8;
						c96 += 4, c96_p_1 += 4, c96_p_0 += 4){
						// y = ph_y, x = 17, h = 1, w = 1, c = 4, f = 32
						// T (x, 17) (17 / 1)
						for (x = x72, xp_1 = x72_p_0, xp_0 = 0;
							x < x72 + 17;
							x += 1, xp_1 += 1, xp_0 += 1){
									mem_vec_830 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f]);
									mem_vec_831 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f + 16]);
									mem_vec_832 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
									mem_vec_833 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16]);
									mem_vec_834 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
									mem_vec_835 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f + 16]);
									mem_vec_836 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
									mem_vec_837 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f + 16]);
									mem_vec_838 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
									mem_vec_839 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16]);
									mem_vec_840 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
									mem_vec_841 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f + 16]);
									mem_vec_842 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
									mem_vec_843 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 16]);
									mem_vec_844 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 7) + f]);
									mem_vec_845 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 7) + f + 16]);
									mem_vec_846 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 8) + f]);
									mem_vec_847 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 8) + f + 16]);
									mem_vec_848 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f]);
									mem_vec_849 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16]);
									// y = ph_y, x = 1, h = 1, w = 1, c = 4, f = 32
									// T (c, 4) (4 / 1)
									for (c = c96, cp_2 = c96_p_1, cp_1 = c96_p_0, cp_0 = 0;
										c < c96 + 4;
										c += 1, cp_2 += 1, cp_1 += 1, cp_0 += 1){
										scal_0 = input[(C * (Y + H - 1)) * (x + w) + C * (y + h) + c];
										vec_1 = _mm512_set1_ps(scal_0);
										vec_2 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * h + F * c + f]);

										vec_0 = _mm512_fmadd_ps(vec_1, vec_2, mem_vec_830);
										mem_vec_830 = vec_0;

										vec_4 = _mm512_loadu_ps(&params[((F * C) * H) * w + (F * C) * h + F * c + f + 16]);

										vec_3 = _mm512_fmadd_ps(vec_1, vec_4, mem_vec_831);
										mem_vec_831 = vec_3;
										scal_1 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h) + c];
										vec_6 = _mm512_set1_ps(scal_1);


										vec_5 = _mm512_fmadd_ps(vec_6, vec_2, mem_vec_832);
										mem_vec_832 = vec_5;



										vec_7 = _mm512_fmadd_ps(vec_6, vec_4, mem_vec_833);
										mem_vec_833 = vec_7;
										scal_2 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 2 + h) + c];
										vec_9 = _mm512_set1_ps(scal_2);


										vec_8 = _mm512_fmadd_ps(vec_9, vec_2, mem_vec_834);
										mem_vec_834 = vec_8;



										vec_10 = _mm512_fmadd_ps(vec_9, vec_4, mem_vec_835);
										mem_vec_835 = vec_10;
										scal_3 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 3 + h) + c];
										vec_12 = _mm512_set1_ps(scal_3);


										vec_11 = _mm512_fmadd_ps(vec_12, vec_2, mem_vec_836);
										mem_vec_836 = vec_11;



										vec_13 = _mm512_fmadd_ps(vec_12, vec_4, mem_vec_837);
										mem_vec_837 = vec_13;
										scal_4 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 4 + h) + c];
										vec_15 = _mm512_set1_ps(scal_4);


										vec_14 = _mm512_fmadd_ps(vec_15, vec_2, mem_vec_838);
										mem_vec_838 = vec_14;



										vec_16 = _mm512_fmadd_ps(vec_15, vec_4, mem_vec_839);
										mem_vec_839 = vec_16;
										scal_5 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 5 + h) + c];
										vec_18 = _mm512_set1_ps(scal_5);


										vec_17 = _mm512_fmadd_ps(vec_18, vec_2, mem_vec_840);
										mem_vec_840 = vec_17;



										vec_19 = _mm512_fmadd_ps(vec_18, vec_4, mem_vec_841);
										mem_vec_841 = vec_19;
										scal_6 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 6 + h) + c];
										vec_21 = _mm512_set1_ps(scal_6);


										vec_20 = _mm512_fmadd_ps(vec_21, vec_2, mem_vec_842);
										mem_vec_842 = vec_20;



										vec_22 = _mm512_fmadd_ps(vec_21, vec_4, mem_vec_843);
										mem_vec_843 = vec_22;
										scal_7 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 7 + h) + c];
										vec_24 = _mm512_set1_ps(scal_7);


										vec_23 = _mm512_fmadd_ps(vec_24, vec_2, mem_vec_844);
										mem_vec_844 = vec_23;



										vec_25 = _mm512_fmadd_ps(vec_24, vec_4, mem_vec_845);
										mem_vec_845 = vec_25;
										scal_8 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 8 + h) + c];
										vec_27 = _mm512_set1_ps(scal_8);


										vec_26 = _mm512_fmadd_ps(vec_27, vec_2, mem_vec_846);
										mem_vec_846 = vec_26;



										vec_28 = _mm512_fmadd_ps(vec_27, vec_4, mem_vec_847);
										mem_vec_847 = vec_28;
										scal_9 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 9 + h) + c];
										vec_30 = _mm512_set1_ps(scal_9);


										vec_29 = _mm512_fmadd_ps(vec_30, vec_2, mem_vec_848);
										mem_vec_848 = vec_29;



										vec_31 = _mm512_fmadd_ps(vec_30, vec_4, mem_vec_849);
										mem_vec_849 = vec_31;
									}
								_mm512_storeu_ps(&output[(F * Y) * x + F * y + f], mem_vec_830);
								_mm512_storeu_ps(&output[(F * Y) * x + F * y + f + 16], mem_vec_831);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_832);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 16], mem_vec_833);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_834);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f + 16], mem_vec_835);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f], mem_vec_836);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f + 16], mem_vec_837);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_838);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16], mem_vec_839);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f], mem_vec_840);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f + 16], mem_vec_841);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f], mem_vec_842);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 16], mem_vec_843);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 7) + f], mem_vec_844);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 7) + f + 16], mem_vec_845);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 8) + f], mem_vec_846);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 8) + f + 16], mem_vec_847);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f], mem_vec_848);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16], mem_vec_849);
						}
					}
				}
			}
		}
}


}