							...
							for (h = h60, hp_0 = 0;h < h60 + 3;h += 1, hp_0 += 1){
								...
								for (w = w60, wp_0 = 0;w < w60 + 3;w += 1, wp_0 += 1){
									...
									for (x = x90, xp_1 = x90_p_0, xp_0 = 0;x < x90 + 1;x += 1, xp_1 += 1, xp_0 += 1){
												...
												mem_vec_1102 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 32]);
												mem_vec_1103 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 48]);
												// y = ph_y, x = 1, h = 1, w = 1, c = 256, f = 64
												// T (c, 256) (256 / 1)
											...
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 32], mem_vec_1102);
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 48], mem_vec_1103);
									}
								}
							}
}
