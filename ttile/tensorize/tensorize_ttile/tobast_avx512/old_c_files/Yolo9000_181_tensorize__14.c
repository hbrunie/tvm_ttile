...
fp_2 = 0;
							for (w = w188, wp_0 = 0;w < w188 + 3;w += 1, wp_0 += 1){
								...
								for (x = x282, xp_1 = x282_p_0, xp_0 = 0;x < x282 + 1;x += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_2548 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 7) + f]);
											mem_vec_2549 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 7) + f + 16]);
											// y = ph_y, x = 1, h = 3, w = 1, c = 512, f = 32
											// T (c, 512) (512 / 1)
											for (c = c328, cp_1 = c328_p_0, cp_0 = 0;c < c328 + 512;c += 1, cp_1 += 1, cp_0 += 1){
										...
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 7) + f], mem_vec_2548);
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 7) + f + 16], mem_vec_2549);
								}
							}
}
