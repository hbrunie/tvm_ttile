...
fp_2 = 0;
							for (h = h338, hp_0 = 0;h < h338 + 3;h += 1, hp_0 += 1){
								...
								for (x = x756, xp_1 = x756_p_0, xp_0 = 0;x < x756 + 1;x += 1, xp_1 += 1, xp_0 += 1){
											mem_vec_5442 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * y + f]);
											mem_vec_5443 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f]);
											mem_vec_5444 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f]);
											mem_vec_5445 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f]);
											// y = ph_y, x = 1, h = 1, w = 3, c = 2, f = 16
											// T (c, 2) (2 / 1)
										...
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * y + f], mem_vec_5442);
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f], mem_vec_5443);
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f], mem_vec_5444);
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f], mem_vec_5445);
								}
							}
}
