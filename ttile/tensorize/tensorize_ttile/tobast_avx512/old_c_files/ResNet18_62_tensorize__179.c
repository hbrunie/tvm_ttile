						...
						for (h = h156, hp_0 = 0;h < h156 + 3;h += 1, hp_0 += 1){
							...
							for (w = w142, wp_0 = 0;w < w142 + 3;w += 1, wp_0 += 1){
								...
								for (x = x288, xp_1 = x288_p_0, xp_0 = 0;x < x288 + 2;x += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_1522 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f]);
											mem_vec_1523 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f + 16]);
											...
											for (c = c294, cp_1 = c294_p_0, cp_0 = 0;c < c294 + 128;c += 1, cp_1 += 1, cp_0 += 1){
												...
												vec_29 = _mm512_fmadd_ps(vec_30, vec_2, mem_vec_1522);
												mem_vec_1522 = vec_29;
												vec_31 = _mm512_fmadd_ps(vec_30, vec_4, mem_vec_1523);
												mem_vec_1523 = vec_31;
											}
										...
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f], mem_vec_1522);
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f + 16], mem_vec_1523);
								}
							}
						}
}
