							...
							for (w = w208, wp_0 = 0;
								...
								for (x = x396, xp_1 = x396_p_0, xp_0 = 0;
									...
									x += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_2284 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f]);
											mem_vec_2285 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16]);
											// y = ph_y, x = 1, h = 1, w = 1, c = 4, f = 32
											// T (c, 4) (4 / 1)
											for (c = c404, cp_1 = c404_p_0, cp_0 = 0;
												c < c404 + 4;
										...
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f], mem_vec_2284);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16], mem_vec_2285);
								}
							}
						}
					}
