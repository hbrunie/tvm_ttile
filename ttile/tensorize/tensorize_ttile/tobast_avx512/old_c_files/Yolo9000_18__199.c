...
[V f; U (1, f); ULambda y; U (3, h); U (3, w); T (32, c); Hoist_vars [c];
  ...
  T (4, c); T (8, f)]
*/
IND_TYPE c, cp_0, c1158_p_0, c1159_p_0, cp_1, c1158_p_1, cp_2, c1158, c1159, f, fp_0, f1208_p_0, fp_1, f1208, x, xp_0, x1014_p_0, xp_1, x1014, y, yp_0;
...
// T (f, 8) (1024 / 128)
for (f1208 = f1209, f1208_p_0 = 0;
	f1208 < f1209 + 1024;
	f1208 += 128, f1208_p_0 += 128){
	...
	for (c1159 = c1160, c1159_p_0 = 0;
		...
		// T (f, 8) (128 / 16)
		for (f = f1208, fp_1 = f1208_p_0, fp_0 = 0;
			f < f1208 + 128;
				...
				for (y = y676, yp_0 = 0;
					...
					for (x1014 = x1015, x1014_p_0 = 0;
						...
						for (c1158 = c1159, c1158_p_1 = c1159_p_0, c1158_p_0 = 0;
							c1158 < c1159 + 128;
							c1158 += 32, c1158_p_1 += 32, c1158_p_0 += 32){
							// y = ph_y, x = 1, h = 3, w = 3, c = 32, f = 16
							// T (x, 1) (1 / 1)
							for (x = x1014, xp_1 = x1014_p_0, xp_0 = 0;
								x < x1014 + 1;
								x += 1, xp_1 += 1, xp_0 += 1){
										mem_vec_8958 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f]);
										mem_vec_8959 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
										mem_vec_8960 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
										mem_vec_8961 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
										mem_vec_8962 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
										// y = ph_y, x = 1, h = 3, w = 3, c = 32, f = 16
										// T (c, 32) (32 / 1)
										for (c = c1158, cp_2 = c1158_p_1, cp_1 = c1158_p_0, cp_0 = 0;
											...
											vec_3 = _mm512_fmadd_ps(vec_4, vec_2, mem_vec_8959);
											mem_vec_8959 = vec_3;
					...
					for (x1014 = x1015, x1014_p_0 = 0;
						...
						for (c1158 = c1159, c1158_p_1 = c1159_p_0, c1158_p_0 = 0;
							...
							for (x = x1014, xp_1 = x1014_p_0, xp_0 = 0;
								x < x1014 + 1;
								x += 1, xp_1 += 1, xp_0 += 1){
										mem_vec_8963 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f]);
										mem_vec_8964 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
										mem_vec_8965 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
										mem_vec_8966 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
										mem_vec_8967 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
										mem_vec_8968 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
										mem_vec_8969 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
										// y = ph_y, x = 1, h = 3, w = 3, c = 32, f = 16
										// T (c, 32) (32 / 1)
										for (c = c1158, cp_2 = c1158_p_1, cp_1 = c1158_p_0, cp_0 = 0;
											c < c1158 + 32;
											...
											vec_0 = _mm512_fmadd_ps(vec_1, vec_2, mem_vec_8963);
											mem_vec_8963 = vec_0;
											scal_1 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h) + c];
											vec_4 = _mm512_set1_ps(scal_1);


											vec_3 = _mm512_fmadd_ps(vec_4, vec_2, mem_vec_8964);
											mem_vec_8964 = vec_3;
											scal_2 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 2 + h) + c];
											vec_6 = _mm512_set1_ps(scal_2);


											vec_5 = _mm512_fmadd_ps(vec_6, vec_2, mem_vec_8965);
											mem_vec_8965 = vec_5;
											scal_3 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 3 + h) + c];
											vec_8 = _mm512_set1_ps(scal_3);


											vec_7 = _mm512_fmadd_ps(vec_8, vec_2, mem_vec_8966);
											mem_vec_8966 = vec_7;
											scal_4 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 4 + h) + c];
											vec_10 = _mm512_set1_ps(scal_4);


											vec_9 = _mm512_fmadd_ps(vec_10, vec_2, mem_vec_8967);
											mem_vec_8967 = vec_9;
											...
											vec_105 = _mm512_fmadd_ps(vec_106, vec_107, mem_vec_8963);
											mem_vec_8963 = vec_105;
											scal_50 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 1 + h + 1) + c];
											vec_109 = _mm512_set1_ps(scal_50);


											vec_108 = _mm512_fmadd_ps(vec_109, vec_107, mem_vec_8964);
											mem_vec_8964 = vec_108;
											scal_51 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 2 + h + 1) + c];
											vec_111 = _mm512_set1_ps(scal_51);


											vec_110 = _mm512_fmadd_ps(vec_111, vec_107, mem_vec_8965);
											mem_vec_8965 = vec_110;
											scal_52 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 3 + h + 1) + c];
											vec_113 = _mm512_set1_ps(scal_52);


											vec_112 = _mm512_fmadd_ps(vec_113, vec_107, mem_vec_8966);
											mem_vec_8966 = vec_112;
											scal_53 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 4 + h + 1) + c];
											vec_115 = _mm512_set1_ps(scal_53);


											vec_114 = _mm512_fmadd_ps(vec_115, vec_107, mem_vec_8967);
											mem_vec_8967 = vec_114;
											scal_54 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 5 + h + 1) + c];
											vec_117 = _mm512_set1_ps(scal_54);


											vec_116 = _mm512_fmadd_ps(vec_117, vec_107, mem_vec_8968);
											mem_vec_8968 = vec_116;
											scal_55 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 6 + h + 1) + c];
											vec_119 = _mm512_set1_ps(scal_55);


											vec_118 = _mm512_fmadd_ps(vec_119, vec_107, mem_vec_8969);
											mem_vec_8969 = vec_118;
											...
											vec_120 = _mm512_fmadd_ps(vec_121, vec_122, mem_vec_8963);
											mem_vec_8963 = vec_120;
											scal_57 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 1 + h + 2) + c];
											vec_124 = _mm512_set1_ps(scal_57);


											vec_123 = _mm512_fmadd_ps(vec_124, vec_122, mem_vec_8964);
											mem_vec_8964 = vec_123;
											scal_58 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 2 + h + 2) + c];
											vec_126 = _mm512_set1_ps(scal_58);


											vec_125 = _mm512_fmadd_ps(vec_126, vec_122, mem_vec_8965);
											mem_vec_8965 = vec_125;
											scal_59 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 3 + h + 2) + c];
											vec_128 = _mm512_set1_ps(scal_59);


											vec_127 = _mm512_fmadd_ps(vec_128, vec_122, mem_vec_8966);
											mem_vec_8966 = vec_127;
											scal_60 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 4 + h + 2) + c];
											vec_130 = _mm512_set1_ps(scal_60);


											vec_129 = _mm512_fmadd_ps(vec_130, vec_122, mem_vec_8967);
											mem_vec_8967 = vec_129;
											scal_61 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 5 + h + 2) + c];
											vec_132 = _mm512_set1_ps(scal_61);


											vec_131 = _mm512_fmadd_ps(vec_132, vec_122, mem_vec_8968);
											mem_vec_8968 = vec_131;
											scal_62 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 6 + h + 2) + c];
											vec_134 = _mm512_set1_ps(scal_62);


											vec_133 = _mm512_fmadd_ps(vec_134, vec_122, mem_vec_8969);
											mem_vec_8969 = vec_133;
										}
									_mm512_storeu_ps(&output[(F * Y) * x + F * y + f], mem_vec_8963);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_8964);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_8965);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f], mem_vec_8966);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_8967);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f], mem_vec_8968);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f], mem_vec_8969);
							}
						}
					}
				}
