						...
						for (w = w20, wp_0 = 0;
							...
							for (x = x30, xp_1 = x30_p_0, xp_0 = 0;
								...
								x += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_234 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 32]);
										mem_vec_235 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 48]);
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 32], mem_vec_234);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 48], mem_vec_235);
							}
						}
					}
				}
