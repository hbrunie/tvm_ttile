...
x202_p_1 = 0;
					for (h = h72, hp_0 = 0;h < h72 + 3;h += 1, hp_0 += 1){
						...
						// T (x, 14) (14 / 1)
						for (x = x202, xp_2 = x202_p_1, xp_1 = x202_p_0, xp_0 = 0;x < x202 + 14;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
									mem_vec_858 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * y + f]);
									mem_vec_859 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f]);
									mem_vec_860 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f]);
									mem_vec_861 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f]);
									// y = 4, x = 1, h = 1, w = 3, c = 8, f = 16
									// T (c, 8) (8 / 1)
									for (c = c190, cp_1 = c190_p_0, cp_0 = 0;c < c190 + 8;c += 1, cp_1 += 1, cp_0 += 1){
								...
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f], mem_vec_859);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f], mem_vec_860);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f], mem_vec_861);
						}
					}
}
