...
// T (x, 8) (272 / 34)
for (x1785 = x1786, x1785_p_0 = 0;
	x1785 < x1786 + 272;
	x1785 += 34, x1785_p_0 += 34){
	// y = 272, x = 34, h = 3, w = 3, c = 32, f = 64
	// T (y, 16) (272 / 17)
	for (y764 = y765, y764_p_0 = 0;
		y764 < y765 + 272;
		y764 += 17, y764_p_0 += 17){
		// y = 17, x = 34, h = 3, w = 3, c = 32, f = 64
		// T (x, 2) (34 / 17)
		for (x1784 = x1785, x1784_p_1 = x1785_p_0, x1784_p_0 = 0;
			x1784 < x1785 + 34;
			x1784 += 17, x1784_p_1 += 17, x1784_p_0 += 17){
			// y = 17, x = 17, h = 3, w = 3, c = 32, f = 64
			// T (f, 4) (64 / 16)
			for (f = f642, fp_0 = 0;
				f < f642 + 64;
				f += 16, fp_0 += 16){
					for (y = y764, yp_1 = y764_p_0, yp_0 = 0;
						y < y764 + 10;
						y += 5, yp_1 += 5, yp_0 += 5){
						...
						for (h = h532, hp_0 = 0;
							...
							// T (x, 17) (17 / 1)
							for (x = x1784, xp_2 = x1784_p_1, xp_1 = x1784_p_0, xp_0 = 0;
								x < x1784 + 17;
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										mem_vec_7100 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f]);
										mem_vec_7101 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
										mem_vec_7102 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
										mem_vec_7103 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_7101);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_7102);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f], mem_vec_7103);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_7104);
							}
						}
					}
					for (y = y764 + 10, yp_1 = y764_p_0, yp_0 = 0;
						y < y764 + 10 + 7;
						y += 7, yp_1 += 7, yp_0 += 7){
						// y = ph_y, x = 17, h = 3, w = 3, c = 32, f = 16
						// T (h, 3) (3 / 1)
						for (h = h532, hp_0 = 0;
							...
							for (x = x1784, xp_2 = x1784_p_1, xp_1 = x1784_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_7107 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
										mem_vec_7108 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
										mem_vec_7109 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
										mem_vec_7110 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
										mem_vec_7111 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
										// y = ph_y, x = 1, h = 1, w = 3, c = 32, f = 16
										// T (c, 32) (32 / 1)
										for (c = c1056, cp_0 = 0;
											c < c1056 + 32;
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_7107);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f], mem_vec_7108);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_7109);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f], mem_vec_7110);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f], mem_vec_7111);
							}
						}
					}
			}
