						...
						for (h = h172, hp_0 = 0;h < h172 + 3;h += 1, hp_0 += 1){
							...
							for (w = w158, wp_0 = 0;w < w158 + 3;w += 1, wp_0 += 1){
								...
								for (x = x342, xp_1 = x342_p_0, xp_0 = 0;x < x342 + 1;x += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_1726 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f]);
											mem_vec_1727 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16]);
											// y = ph_y, x = 1, h = 1, w = 1, c = 128, f = 32
											// T (c, 128) (128 / 1)
										...
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f], mem_vec_1726);
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16], mem_vec_1727);
								}
							}
						}
}
