							...
							for (w = w142, wp_0 = 0;
								...
								for (x = x288, xp_1 = x288_p_0, xp_0 = 0;
									...
									x += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_1522 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f]);
											mem_vec_1523 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16]);
											// y = ph_y, x = 1, h = 1, w = 1, c = 128, f = 32
											// T (c, 128) (128 / 1)
											for (c = c294, cp_1 = c294_p_0, cp_0 = 0;
												c < c294 + 128;
										...
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f], mem_vec_1522);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 9) + f + 16], mem_vec_1523);
								}
							}
						}
					}
