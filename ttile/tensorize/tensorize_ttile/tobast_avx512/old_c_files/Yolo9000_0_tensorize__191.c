...
[V f; U (1, f); U (4, y); U (3, w); T (3, c); Hoist_vars [c]; T (17, x);
  T (3, h); T (68, y); T (16, x); T (2, y); T (2, f); T (2, x)]
*/
IND_TYPE c, cp_0, f, fp_0, h, hp_0, x, xp_0, x34_p_0, x35_p_0, xp_1, x34_p_1, xp_2, x34, x35, y, yp_0, y36_p_0, yp_1, y36;
IND_TYPE y37 = 0;
IND_TYPE x36 = 0;
IND_TYPE h12 = 0;
IND_TYPE w = 0;
...
yp_1 = 0;
					for (h = h12, hp_0 = 0;h < h12 + 3;h += 1, hp_0 += 1){
						...
						// T (x, 17) (17 / 1)
						for (x = x34, xp_2 = x34_p_1, xp_1 = x34_p_0, xp_0 = 0;x < x34 + 17;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
									mem_vec_112 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * y + f]);
									mem_vec_113 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f]);
									mem_vec_114 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f]);
									mem_vec_115 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f]);
								...
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f], mem_vec_113);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f], mem_vec_114);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f], mem_vec_115);
						}
					}
}
