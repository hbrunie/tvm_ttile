...
x123 = 0;
x123_p_0 = 0;
		for (f = f82, fp_0 = 0;f < f82 + 64;f += 32, fp_0 += 32){
				for (y = y123, yp_1 = y123_p_0, yp_0 = 0;y < y123 + 8;y += 8, yp_1 += 8, yp_0 += 8){
					...
					for (x = x123, xp_1 = x123_p_0, xp_0 = 0;x < x123 + 2;x += 1, xp_1 += 1, xp_0 += 1){
								...
								// T (c, 128) (128 / 1)
								for (c = c82, cp_0 = 0;c < c82 + 128;c += 1, cp_0 += 1){
				...
				for (y = y123 + 8, yp_1 = y123_p_0, yp_0 = 0;y < y123 + 8 + 26;y += 13, yp_1 += 13, yp_0 += 13){
					// y = ph_y, x = 2, h = 1, w = 1, c = 128, f = 32
					// T (x, 2) (2 / 1)
					for (x = x123, xp_1 = x123_p_0, xp_0 = 0;x < x123 + 2;x += 1, xp_1 += 1, xp_0 += 1){
								...
								mem_vec_1722 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 11) + f]);
								mem_vec_1723 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 11) + f + 16]);
								mem_vec_1724 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f]);
								mem_vec_1725 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f + 16]);
								// y = ph_y, x = 1, h = 1, w = 1, c = 128, f = 32
								// T (c, 128) (128 / 1)
								for (c = c82, cp_0 = 0;c < c82 + 128;c += 1, cp_0 += 1){
									scal_0 = input[strideA1 * (x + w) + strideA2 * (y + h) + c];
							...
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 11) + f], mem_vec_1722);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 11) + f + 16], mem_vec_1723);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f], mem_vec_1724);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f + 16], mem_vec_1725);
					}
				}
		}
}
