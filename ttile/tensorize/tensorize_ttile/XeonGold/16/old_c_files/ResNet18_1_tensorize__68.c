...
[V f; U (2, f); U (14, y); T (3, c); Hoist_vars [c]; T (4, x); T (1, c);
  T (7, w); T (7, h); T (28, x); T (2, y); T (2, f); T (4, y)]
*/
IND_TYPE c, cp_0, c81_p_0, cp_1, c81, f, fp_0, h, hp_0, w, wp_0, x, xp_0, x76_p_0, xp_1, x76, y, yp_0, y81_p_0, yp_1, y81;
IND_TYPE y82 = 0;
IND_TYPE x77 = 0;
IND_TYPE h54 = 0;
IND_TYPE w54 = 0;
				...
				for (h = h54, hp_0 = 0;h < h54 + 7;h += 1, hp_0 += 1){
					...
					for (w = w54, wp_0 = 0;w < w54 + 7;w += 1, wp_0 += 1){
						...
						for (c81 = c82, c81_p_0 = 0;c81 < c82 + 3;c81 += 3, c81_p_0 += 3){
							// y = 14, x = 4, h = 1, w = 1, c = 3, f = 32
							// T (x, 4) (4 / 1)
							for (x = x76, xp_1 = x76_p_0, xp_0 = 0;x < x76 + 4;x += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_780 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f]);
										mem_vec_781 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f + 16]);
										mem_vec_782 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f]);
										mem_vec_783 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16]);
									...
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f], mem_vec_780);
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f + 16], mem_vec_781);
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f], mem_vec_782);
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16], mem_vec_783);
							}
						}
					}
				}
