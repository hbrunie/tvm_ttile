	...
	for (x229 = x230, x229_p_0 = 0;
		...
		for (f = f114, fp_0 = 0;
			...
			f += 32, fp_0 += 32){
				...
				for (y = y163 + 20, yp_1 = y163_p_0, yp_0 = 0;
					y < y163 + 20 + 14;
						...
						for (c171 = c172, c171_p_0 = 0;
							...
							for (x = x228, xp_2 = x228_p_1, xp_1 = x228_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_2632 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
										mem_vec_2633 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
										mem_vec_2634 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f]);
										mem_vec_2635 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16]);
										// y = ph_y, x = 1, h = 1, w = 1, c = 64, f = 32
										// T (c, 64) (64 / 1)
										for (c = c171, cp_1 = c171_p_0, cp_0 = 0;
											c < c171 + 64;
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_2632);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_2633);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f], mem_vec_2634);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16], mem_vec_2635);
							}
						}
					}
				}
