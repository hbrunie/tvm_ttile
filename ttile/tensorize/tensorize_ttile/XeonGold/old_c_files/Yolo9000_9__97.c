						...
						for (c51 = c52, c51_p_0 = 0;
							...
							for (x = x68, xp_2 = x68_p_1, xp_1 = x68_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_930 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f + 32]);
										mem_vec_931 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f + 48]);
										mem_vec_932 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
										mem_vec_933 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 16]);
										mem_vec_934 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 32]);
										mem_vec_935 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 48]);
										// y = ph_y, x = 1, h = 1, w = 1, c = 64, f = 64
										// T (c, 64) (64 / 1)
										for (c = c51, cp_1 = c51_p_0, cp_0 = 0;
											c < c51 + 64;
											...
											vec_32 = _mm512_fmadd_ps(vec_30, vec_6, mem_vec_930);
											mem_vec_930 = vec_32;



											vec_33 = _mm512_fmadd_ps(vec_30, vec_8, mem_vec_931);
											mem_vec_931 = vec_33;
											scal_6 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 6 + h) + c];
											vec_35 = _mm512_set1_ps(scal_6);


											vec_34 = _mm512_fmadd_ps(vec_35, vec_2, mem_vec_932);
											mem_vec_932 = vec_34;



											vec_36 = _mm512_fmadd_ps(vec_35, vec_4, mem_vec_933);
											mem_vec_933 = vec_36;



											vec_37 = _mm512_fmadd_ps(vec_35, vec_6, mem_vec_934);
											mem_vec_934 = vec_37;



											vec_38 = _mm512_fmadd_ps(vec_35, vec_8, mem_vec_935);
											mem_vec_935 = vec_38;
										}
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f + 32], mem_vec_930);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f + 48], mem_vec_931);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f], mem_vec_932);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 16], mem_vec_933);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 32], mem_vec_934);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 48], mem_vec_935);
							}
						}
					}
				}
