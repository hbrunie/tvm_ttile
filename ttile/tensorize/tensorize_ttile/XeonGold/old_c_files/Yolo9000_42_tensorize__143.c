						...
						for (w = w76, wp_0 = 0;w < w76 + 3;w += 1, wp_0 += 1){
							...
							for (x = x144, xp_2 = x144_p_1, xp_1 = x144_p_0, xp_0 = 0;x < x144 + 17;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
								...
								for (h = h72, hp_0 = 0;h < h72 + 3;h += 1, hp_0 += 1){
											...
											mem_vec_1620 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 14) + f]);
											mem_vec_1621 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 14) + f + 16]);
										...
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 14) + f], mem_vec_1620);
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 14) + f + 16], mem_vec_1621);
								}
							}
						}
}
