	...
	for (x301 = x302, x301_p_0 = 0;
		...
		for (f = f150, fp_0 = 0;
			...
			f += 32, fp_0 += 32){
				...
				for (y = y217 + 22, yp_1 = y217_p_0, yp_0 = 0;
					y < y217 + 22 + 12;
						...
						for (c225 = c226, c225_p_0 = 0;
							...
							for (x = x300, xp_2 = x300_p_1, xp_1 = x300_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_3468 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f]);
										mem_vec_3469 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16]);
										mem_vec_3470 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f]);
										mem_vec_3471 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16]);
										// y = ph_y, x = 1, h = 1, w = 1, c = 64, f = 32
										// T (c, 64) (64 / 1)
										for (c = c225, cp_1 = c225_p_0, cp_0 = 0;
											c < c225 + 64;
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f], mem_vec_3468);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16], mem_vec_3469);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f], mem_vec_3470);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16], mem_vec_3471);
							}
						}
					}
				}
