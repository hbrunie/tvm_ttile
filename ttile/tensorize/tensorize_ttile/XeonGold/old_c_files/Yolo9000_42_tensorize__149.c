						...
						for (w = w34, wp_0 = 0;w < w34 + 3;w += 1, wp_0 += 1){
							...
							for (x = x68, xp_2 = x68_p_1, xp_1 = x68_p_0, xp_0 = 0;x < x68 + 34;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
								...
								for (h = h34, hp_0 = 0;h < h34 + 3;h += 1, hp_0 += 1){
											...
											mem_vec_880 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 10) + f]);
											mem_vec_881 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 10) + f + 16]);
										...
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 10) + f], mem_vec_880);
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 10) + f + 16], mem_vec_881);
								}
							}
						}
}
