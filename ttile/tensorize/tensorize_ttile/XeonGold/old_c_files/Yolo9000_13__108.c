	...
	for (x197 = x198, x197_p_0 = 0;
		...
		for (f = f98, fp_0 = 0;
			...
			f += 32, fp_0 += 32){
				...
				for (y = y139 + 20, yp_1 = y139_p_0, yp_0 = 0;
					y < y139 + 20 + 14;
						...
						for (c147 = c148, c147_p_0 = 0;
							...
							for (x = x196, xp_2 = x196_p_1, xp_1 = x196_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_2248 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
										mem_vec_2249 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
										mem_vec_2250 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f]);
										mem_vec_2251 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16]);
										// y = ph_y, x = 1, h = 1, w = 1, c = 64, f = 32
										// T (c, 64) (64 / 1)
										for (c = c147, cp_1 = c147_p_0, cp_0 = 0;
											c < c147 + 64;
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_2248);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_2249);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f], mem_vec_2250);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16], mem_vec_2251);
							}
						}
					}
				}
