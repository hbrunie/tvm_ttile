	...
	for (x185 = x186, x185_p_0 = 0;
		...
		for (f = f92, fp_0 = 0;
			...
			f += 32, fp_0 += 32){
				...
				for (y = y130 + 20, yp_1 = y130_p_0, yp_0 = 0;
					y < y130 + 20 + 14;
						...
						for (c138 = c139, c138_p_0 = 0;
							...
							for (x = x184, xp_2 = x184_p_1, xp_1 = x184_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_2104 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
										mem_vec_2105 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
										mem_vec_2106 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f]);
										mem_vec_2107 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16]);
										// y = ph_y, x = 1, h = 1, w = 1, c = 512, f = 32
										// T (c, 512) (512 / 1)
										for (c = c138, cp_1 = c138_p_0, cp_0 = 0;
											c < c138 + 512;
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_2104);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_2105);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f], mem_vec_2106);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16], mem_vec_2107);
							}
						}
					}
				}
