					...
					for (w = w133, wp_0 = 0;w < w133 + 3;w += 1, wp_0 += 1){
						...
						for (x = x220, xp_1 = x220_p_0, xp_0 = 0;x < x220 + 7;x += 1, xp_1 += 1, xp_0 += 1){
							...
							for (h = h132, hp_0 = 0;h < h132 + 3;h += 1, hp_0 += 1){
										...
										mem_vec_1878 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f]);
										mem_vec_1879 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16]);
									...
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f], mem_vec_1878);
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16], mem_vec_1879);
							}
						}
					}
}
