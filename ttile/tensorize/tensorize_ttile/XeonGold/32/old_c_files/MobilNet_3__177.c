	...
	x227 += 14, x227_p_0 += 14){
		for (y = y138, yp_0 = 0;
			...
			for (x226 = x227, x226_p_1 = x227_p_0, x226_p_0 = 0;
				x226 < x227 + 14;
				x226 += 14, x226_p_1 += 14, x226_p_0 += 14){
				// y = ph_y, x = 14, h = 3, w = 3, c = 128, f = 128
				// T (f, 4) (128 / 32)
				for (f = f222, fp_0 = 0;
					f < f222 + 128;
							...
							for (x = x226, xp_2 = x226_p_1, xp_1 = x226_p_0, xp_0 = 0;
								...
								for (h = h93, hp_0 = 0;
									...
									h += 1, hp_0 += 1){
											...
											mem_vec_1502 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 14) + f]);
											mem_vec_1503 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 14) + f + 16]);
											// y = ph_y, x = 1, h = 1, w = 1, c = 32, f = 32
											// T (c, 32) (32 / 1)
											for (c = c207, cp_1 = c207_p_0, cp_0 = 0;
												c < c207 + 32;
										...
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 14) + f], mem_vec_1502);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 14) + f + 16], mem_vec_1503);
								}
							}
						}
					}
