					...
					for (c60 = c61, c60_p_0 = 0;
						...
						for (x = x80, xp_2 = x80_p_1, xp_1 = x80_p_0, xp_0 = 0;
							...
							x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
									...
									mem_vec_946 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 32]);
									mem_vec_947 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 48]);
									// y = ph_y, x = 1, h = 1, w = 1, c = 256, f = 64
									// T (c, 256) (256 / 1)
									for (c = c60, cp_1 = c60_p_0, cp_0 = 0;
										c < c60 + 256;
								...
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 32], mem_vec_946);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 48], mem_vec_947);
						}
					}
				}
			}
