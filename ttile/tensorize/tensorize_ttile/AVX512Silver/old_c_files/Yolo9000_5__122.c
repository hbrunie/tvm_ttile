...
[V f; U (2, f); ULambda y; T (128, c); Hoist_vars [c]; T (1, x);
  ...
  T (136, x); T (4, y)]
*/
IND_TYPE c, cp_0, f, fp_0, x, xp_0, x336_p_0, xp_1, x336, y, yp_0, y336_p_0, yp_1, y336;
	...
	for (x336 = x337, x336_p_0 = 0;
		...
		for (f = f196, fp_0 = 0;
			...
			f += 32, fp_0 += 32){
				...
				for (y = y336 + 21, yp_1 = y336_p_0, yp_0 = 0;
					y < y336 + 21 + 13;
					...
					for (x = x336, xp_1 = x336_p_0, xp_0 = 0;
						...
						x += 1, xp_1 += 1, xp_0 += 1){
								...
								mem_vec_4674 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f]);
								mem_vec_4675 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16]);
								mem_vec_4676 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
								mem_vec_4677 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
								// y = ph_y, x = 1, h = 1, w = 1, c = 128, f = 32
								// T (c, 128) (128 / 1)
								for (c = c252, cp_0 = 0;
									c < c252 + 128;
									...
									vec_35 = _mm512_fmadd_ps(vec_36, vec_2, mem_vec_4674);
									mem_vec_4674 = vec_35;



									vec_37 = _mm512_fmadd_ps(vec_36, vec_4, mem_vec_4675);
									mem_vec_4675 = vec_37;
									scal_12 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 12 + h) + c];
									vec_39 = _mm512_set1_ps(scal_12);


									vec_38 = _mm512_fmadd_ps(vec_39, vec_2, mem_vec_4676);
									mem_vec_4676 = vec_38;



									vec_40 = _mm512_fmadd_ps(vec_39, vec_4, mem_vec_4677);
									mem_vec_4677 = vec_40;
								}
							...
							_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f], mem_vec_4674);
							_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16], mem_vec_4675);
							_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_4676);
							_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_4677);
					}
				}
		}
	}
