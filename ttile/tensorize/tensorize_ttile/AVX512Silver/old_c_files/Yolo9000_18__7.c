...
[V f; U (1, f); ULambda y; U (3, h); U (3, w); T (32, c); Hoist_vars [c];
  ...
  T (8, c); T (1, f)]
*/
IND_TYPE c, cp_0, c384_p_0, c385_p_0, cp_1, c384_p_1, cp_2, c384, c385, f, fp_0, f288_p_0, fp_1, f288, x, xp_0, y, yp_0;
					...
					for (c384 = c385, c384_p_1 = c385_p_0, c384_p_0 = 0;
						...
						for (x = x192, xp_0 = 0;
							...
							x += 1, xp_0 += 1){
									...
									mem_vec_1867 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
									mem_vec_1868 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
									mem_vec_1869 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
									mem_vec_1870 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
									...
									for (c = c384, cp_2 = c384_p_1, cp_1 = c384_p_0, cp_0 = 0;
										...
										vec_3 = _mm512_fmadd_ps(vec_4, vec_2, mem_vec_1867);
										mem_vec_1867 = vec_3;
				...
				for (y = y192 + 10, yp_0 = 0;
					...
					for (c384 = c385, c384_p_1 = c385_p_0, c384_p_0 = 0;
						...
						for (x = x192, xp_0 = 0;
							x < x192 + 17;
							x += 1, xp_0 += 1){
									mem_vec_1871 = _mm512_loadu_ps(&output[(F * Y) * x + F * y + f]);
									...
									mem_vec_1875 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
									mem_vec_1876 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
									mem_vec_1877 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
									// y = ph_y, x = 1, h = 3, w = 3, c = 32, f = 16
									// T (c, 32) (32 / 1)
									for (c = c384, cp_2 = c384_p_1, cp_1 = c384_p_0, cp_0 = 0;
										c < c384 + 32;
										...
										vec_3 = _mm512_fmadd_ps(vec_4, vec_2, mem_vec_1872);
										mem_vec_1872 = vec_3;
										scal_2 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 2 + h) + c];
										vec_6 = _mm512_set1_ps(scal_2);
										...
										vec_120 = _mm512_fmadd_ps(vec_121, vec_122, mem_vec_1871);
										mem_vec_1871 = vec_120;
										scal_57 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 1 + h + 2) + c];
										vec_124 = _mm512_set1_ps(scal_57);


										vec_123 = _mm512_fmadd_ps(vec_124, vec_122, mem_vec_1872);
										mem_vec_1872 = vec_123;
										scal_58 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 2 + h + 2) + c];
										vec_126 = _mm512_set1_ps(scal_58);


										vec_125 = _mm512_fmadd_ps(vec_126, vec_122, mem_vec_1873);
										mem_vec_1873 = vec_125;
										scal_59 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 3 + h + 2) + c];
										vec_128 = _mm512_set1_ps(scal_59);


										vec_127 = _mm512_fmadd_ps(vec_128, vec_122, mem_vec_1874);
										mem_vec_1874 = vec_127;
										scal_60 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 4 + h + 2) + c];
										vec_130 = _mm512_set1_ps(scal_60);


										vec_129 = _mm512_fmadd_ps(vec_130, vec_122, mem_vec_1875);
										mem_vec_1875 = vec_129;
										scal_61 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 5 + h + 2) + c];
										vec_132 = _mm512_set1_ps(scal_61);


										vec_131 = _mm512_fmadd_ps(vec_132, vec_122, mem_vec_1876);
										mem_vec_1876 = vec_131;
										scal_62 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 6 + h + 2) + c];
										vec_134 = _mm512_set1_ps(scal_62);


										vec_133 = _mm512_fmadd_ps(vec_134, vec_122, mem_vec_1877);
										mem_vec_1877 = vec_133;
									}
								_mm512_storeu_ps(&output[(F * Y) * x + F * y + f], mem_vec_1871);
								...
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_1875);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f], mem_vec_1876);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f], mem_vec_1877);
						}
					}
				}
		}
