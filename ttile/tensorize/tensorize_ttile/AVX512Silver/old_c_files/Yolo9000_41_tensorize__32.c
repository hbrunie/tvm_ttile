						...
						for (w = w30, wp_0 = 0;w < w30 + 3;w += 1, wp_0 += 1){
							...
							for (x = x120, xp_2 = x120_p_1, xp_1 = x120_p_0, xp_0 = 0;x < x120 + 34;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
								...
								for (h = h30, hp_0 = 0;h < h30 + 3;h += 1, hp_0 += 1){
											...
											mem_vec_1344 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f]);
											mem_vec_1345 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f + 16]);
										...
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f], mem_vec_1344);
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 12) + f + 16], mem_vec_1345);
								}
							}
						}
}
