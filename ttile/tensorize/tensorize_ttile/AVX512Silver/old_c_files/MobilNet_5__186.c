...
[V f; U (1, f); U (7, y); U (3, h); U (3, w); T (128, c); Hoist_vars [c];
  T (28, x); T (2, c); T (4, y); T (16, f); T (1, y)]
*/
IND_TYPE c, cp_0, c487_p_0, cp_1, c487, f, fp_0, x, xp_0, y, yp_0, y419_p_0, yp_1, y419;
			...
			for (c487 = c488, c487_p_0 = 0;
				...
				for (x = x427, xp_0 = 0;
					...
					x += 1, xp_0 += 1){
							...
							mem_vec_2713 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
							mem_vec_2714 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
							mem_vec_2715 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
							mem_vec_2716 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
							mem_vec_2717 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
							mem_vec_2718 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
							// y = 7, x = 1, h = 3, w = 3, c = 128, f = 16
							// T (c, 128) (128 / 1)
							for (c = c487, cp_1 = c487_p_0, cp_0 = 0;
								c < c487 + 128;
								c += 1, cp_1 += 1, cp_0 += 1){
								...
								vec_3 = _mm512_fmadd_ps(vec_4, vec_2, mem_vec_2713);
								mem_vec_2713 = vec_3;
								...
								vec_7 = _mm512_fmadd_ps(vec_8, vec_2, mem_vec_2715);
								mem_vec_2715 = vec_7;
								...
								vec_105 = _mm512_fmadd_ps(vec_106, vec_107, mem_vec_2712);
								mem_vec_2712 = vec_105;
								scal_50 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 1 + h + 1) + c];
								vec_109 = _mm512_set1_ps(scal_50);


								vec_108 = _mm512_fmadd_ps(vec_109, vec_107, mem_vec_2713);
								mem_vec_2713 = vec_108;
								scal_51 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 2 + h + 1) + c];
								vec_111 = _mm512_set1_ps(scal_51);


								vec_110 = _mm512_fmadd_ps(vec_111, vec_107, mem_vec_2714);
								mem_vec_2714 = vec_110;
								scal_52 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 3 + h + 1) + c];
								vec_113 = _mm512_set1_ps(scal_52);


								vec_112 = _mm512_fmadd_ps(vec_113, vec_107, mem_vec_2715);
								mem_vec_2715 = vec_112;
								scal_53 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 4 + h + 1) + c];
								vec_115 = _mm512_set1_ps(scal_53);


								vec_114 = _mm512_fmadd_ps(vec_115, vec_107, mem_vec_2716);
								mem_vec_2716 = vec_114;
								scal_54 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 5 + h + 1) + c];
								vec_117 = _mm512_set1_ps(scal_54);


								vec_116 = _mm512_fmadd_ps(vec_117, vec_107, mem_vec_2717);
								mem_vec_2717 = vec_116;
								scal_55 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 6 + h + 1) + c];
								vec_119 = _mm512_set1_ps(scal_55);


								vec_118 = _mm512_fmadd_ps(vec_119, vec_107, mem_vec_2718);
								mem_vec_2718 = vec_118;
								...
								vec_120 = _mm512_fmadd_ps(vec_121, vec_122, mem_vec_2712);
								mem_vec_2712 = vec_120;
								scal_57 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 1 + h + 2) + c];
								vec_124 = _mm512_set1_ps(scal_57);


								vec_123 = _mm512_fmadd_ps(vec_124, vec_122, mem_vec_2713);
								mem_vec_2713 = vec_123;
								scal_58 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 2 + h + 2) + c];
								vec_126 = _mm512_set1_ps(scal_58);


								vec_125 = _mm512_fmadd_ps(vec_126, vec_122, mem_vec_2714);
								mem_vec_2714 = vec_125;
								scal_59 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 3 + h + 2) + c];
								vec_128 = _mm512_set1_ps(scal_59);


								vec_127 = _mm512_fmadd_ps(vec_128, vec_122, mem_vec_2715);
								mem_vec_2715 = vec_127;
								scal_60 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 4 + h + 2) + c];
								vec_130 = _mm512_set1_ps(scal_60);


								vec_129 = _mm512_fmadd_ps(vec_130, vec_122, mem_vec_2716);
								mem_vec_2716 = vec_129;
								scal_61 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 5 + h + 2) + c];
								vec_132 = _mm512_set1_ps(scal_61);


								vec_131 = _mm512_fmadd_ps(vec_132, vec_122, mem_vec_2717);
								mem_vec_2717 = vec_131;
								scal_62 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 6 + h + 2) + c];
								vec_134 = _mm512_set1_ps(scal_62);


								vec_133 = _mm512_fmadd_ps(vec_134, vec_122, mem_vec_2718);
								mem_vec_2718 = vec_133;
							}
						_mm512_storeu_ps(&output[(F * Y) * x + F * y + f], mem_vec_2712);
						_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_2713);
						_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_2714);
						_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f], mem_vec_2715);
						_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_2716);
						_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f], mem_vec_2717);
