...
/*
[V f; U (1, f); U (8, y); U (3, h); U (3, w); T (3, c); Hoist_vars [c];
  T (1, x); T (2, f); T (1, x); T (2, y); T (17, y); T (544, x); T (2, y)]
*/
IND_TYPE c, cp_0, f, fp_0, x, xp_0, x112_p_0, x113_p_0, xp_1, x112_p_1, xp_2, x112, x113, y, yp_0, y106_p_0, y107_p_0, yp_1, y106_p_1, yp_2, y106, y107;
...
for (y107 = y108, y107_p_0 = 0;
	...
	for (x113 = x114, x113_p_0 = 0;
		...
		// T (y, 17) (272 / 16)
		for (y106 = y107, y106_p_1 = y107_p_0, y106_p_0 = 0;
			y106 < y107 + 272;
			y106 += 16, y106_p_1 += 16, y106_p_0 += 16){
			// y = 16, x = 1, h = 3, w = 3, c = 3, f = 32
			// T (y, 2) (16 / 8)
			for (y = y106, yp_2 = y106_p_1, yp_1 = y106_p_0, yp_0 = 0;
				y < y106 + 16;
					...
					for (f = f50, fp_0 = 0;
						...
						for (x = x112, xp_2 = x112_p_1, xp_1 = x112_p_0, xp_0 = 0;
							...
							x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
									...
									mem_vec_217 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
									mem_vec_218 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
									mem_vec_219 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
									mem_vec_220 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
									mem_vec_221 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
									mem_vec_222 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
									mem_vec_223 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 7) + f]);
									// y = 8, x = 1, h = 3, w = 3, c = 3, f = 16
									// T (c, 3) (3 / 1)
									for (c = c62, cp_0 = 0;
										c < c62 + 3;
										...
										vec_3 = _mm512_fmadd_ps(vec_4, vec_2, mem_vec_217);
										mem_vec_217 = vec_3;
										...
										vec_136 = _mm512_fmadd_ps(vec_137, vec_138, mem_vec_216);
										mem_vec_216 = vec_136;
										scal_65 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 1 + h + 2) + c];
										vec_140 = _mm512_set1_ps(scal_65);


										vec_139 = _mm512_fmadd_ps(vec_140, vec_138, mem_vec_217);
										mem_vec_217 = vec_139;
										scal_66 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 2 + h + 2) + c];
										vec_142 = _mm512_set1_ps(scal_66);


										vec_141 = _mm512_fmadd_ps(vec_142, vec_138, mem_vec_218);
										mem_vec_218 = vec_141;
										scal_67 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 3 + h + 2) + c];
										vec_144 = _mm512_set1_ps(scal_67);


										vec_143 = _mm512_fmadd_ps(vec_144, vec_138, mem_vec_219);
										mem_vec_219 = vec_143;
										scal_68 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 4 + h + 2) + c];
										vec_146 = _mm512_set1_ps(scal_68);


										vec_145 = _mm512_fmadd_ps(vec_146, vec_138, mem_vec_220);
										mem_vec_220 = vec_145;
										scal_69 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 5 + h + 2) + c];
										vec_148 = _mm512_set1_ps(scal_69);


										vec_147 = _mm512_fmadd_ps(vec_148, vec_138, mem_vec_221);
										mem_vec_221 = vec_147;
										scal_70 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 6 + h + 2) + c];
										vec_150 = _mm512_set1_ps(scal_70);


										vec_149 = _mm512_fmadd_ps(vec_150, vec_138, mem_vec_222);
										mem_vec_222 = vec_149;
										scal_71 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 7 + h + 2) + c];
										vec_152 = _mm512_set1_ps(scal_71);


										vec_151 = _mm512_fmadd_ps(vec_152, vec_138, mem_vec_223);
										mem_vec_223 = vec_151;
									}
								_mm512_storeu_ps(&output[(F * Y) * x + F * y + f], mem_vec_216);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_217);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_218);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f], mem_vec_219);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_220);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f], mem_vec_221);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f], mem_vec_222);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 7) + f], mem_vec_223);
						}
					}
				}
			}
