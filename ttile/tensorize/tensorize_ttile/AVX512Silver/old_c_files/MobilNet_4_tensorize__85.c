					...
					for (w = w277, wp_0 = 0;w < w277 + 3;w += 1, wp_0 += 1){
						...
						for (x = x438, xp_1 = x438_p_0, xp_0 = 0;x < x438 + 7;x += 1, xp_1 += 1, xp_0 += 1){
							...
							for (h = h269, hp_0 = 0;h < h269 + 3;h += 1, hp_0 += 1){
										...
										mem_vec_2868 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f]);
										mem_vec_2869 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16]);
									...
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f], mem_vec_2868);
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16], mem_vec_2869);
							}
						}
					}
}
