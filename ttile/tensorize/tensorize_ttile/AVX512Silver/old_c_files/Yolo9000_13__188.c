					...
					for (c30 = c31, c30_p_0 = 0;
						...
						for (x = x40, xp_2 = x40_p_1, xp_1 = x40_p_0, xp_0 = 0;
							...
							x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
									...
									mem_vec_502 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f + 32]);
									mem_vec_503 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f + 48]);
									// y = ph_y, x = 1, h = 1, w = 1, c = 128, f = 64
									// T (c, 128) (128 / 1)
									for (c = c30, cp_1 = c30_p_0, cp_0 = 0;
										c < c30 + 128;
								...
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f + 32], mem_vec_502);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f + 48], mem_vec_503);
						}
					}
				}
			}
