...
[V f; U (1, f); U (7, y); U (3, h); U (3, w); T (16, c); Hoist_vars [c];
  T (7, x); T (8, f); T (32, c); T (2, c); T (8, f)]
*/
IND_TYPE c, cp_0, c740_p_0, c741_p_0, cp_1, c740_p_1, cp_2, c740, c741, f, fp_0, f555_p_0, fp_1, f555, x, xp_0;
			...
			for (f = f555, fp_1 = f555_p_0, fp_0 = 0;
				...
				for (x = x370, xp_0 = 0;
					...
					x += 1, xp_0 += 1){
							...
							mem_vec_2893 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
							mem_vec_2894 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
							mem_vec_2895 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
							mem_vec_2896 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
							mem_vec_2897 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
							...
							for (c = c740, cp_2 = c740_p_1, cp_1 = c740_p_0, cp_0 = 0;
								...
								vec_105 = _mm512_fmadd_ps(vec_106, vec_107, mem_vec_2891);
								mem_vec_2891 = vec_105;
								scal_50 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 1 + h + 1) + c];
								vec_109 = _mm512_set1_ps(scal_50);


								vec_108 = _mm512_fmadd_ps(vec_109, vec_107, mem_vec_2892);
								mem_vec_2892 = vec_108;
								scal_51 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 2 + h + 1) + c];
								vec_111 = _mm512_set1_ps(scal_51);


								vec_110 = _mm512_fmadd_ps(vec_111, vec_107, mem_vec_2893);
								mem_vec_2893 = vec_110;
								scal_52 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 3 + h + 1) + c];
								vec_113 = _mm512_set1_ps(scal_52);


								vec_112 = _mm512_fmadd_ps(vec_113, vec_107, mem_vec_2894);
								mem_vec_2894 = vec_112;
								scal_53 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 4 + h + 1) + c];
								vec_115 = _mm512_set1_ps(scal_53);


								vec_114 = _mm512_fmadd_ps(vec_115, vec_107, mem_vec_2895);
								mem_vec_2895 = vec_114;
								scal_54 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 5 + h + 1) + c];
								vec_117 = _mm512_set1_ps(scal_54);


								vec_116 = _mm512_fmadd_ps(vec_117, vec_107, mem_vec_2896);
								mem_vec_2896 = vec_116;
								scal_55 = input[(C * (Y + H - 1)) * (x + w + 2) + C * (y + 6 + h + 1) + c];
								vec_119 = _mm512_set1_ps(scal_55);


								vec_118 = _mm512_fmadd_ps(vec_119, vec_107, mem_vec_2897);
								mem_vec_2897 = vec_118;
