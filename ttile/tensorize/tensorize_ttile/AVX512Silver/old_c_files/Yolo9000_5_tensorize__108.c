...
x315 = 0;
x315_p_0 = 0;
		for (f = f182, fp_0 = 0;f < f182 + 64;f += 32, fp_0 += 32){
				for (y = y315, yp_1 = y315_p_0, yp_0 = 0;y < y315 + 7;y += 7, yp_1 += 7, yp_0 += 7){
					// y = ph_y, x = 2, h = 1, w = 1, c = 128, f = 32
					// T (x, 2) (2 / 1)
					for (x = x315, xp_1 = x315_p_0, xp_0 = 0;x < x315 + 2;x += 1, xp_1 += 1, xp_0 += 1){
								...
								// T (c, 128) (128 / 1)
								for (c = c238, cp_0 = 0;c < c238 + 128;c += 1, cp_0 += 1){
				...
				for (y = y315 + 7, yp_1 = y315_p_0, yp_0 = 0;y < y315 + 7 + 27;y += 9, yp_1 += 9, yp_0 += 9){
					// y = ph_y, x = 2, h = 1, w = 1, c = 128, f = 32
					// T (x, 2) (2 / 1)
					for (x = x315, xp_1 = x315_p_0, xp_0 = 0;x < x315 + 2;x += 1, xp_1 += 1, xp_0 += 1){
								...
								mem_vec_4414 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 7) + f]);
								mem_vec_4415 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 7) + f + 16]);
								mem_vec_4416 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 8) + f]);
								mem_vec_4417 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 8) + f + 16]);
								// y = ph_y, x = 1, h = 1, w = 1, c = 128, f = 32
								// T (c, 128) (128 / 1)
								for (c = c238, cp_0 = 0;c < c238 + 128;c += 1, cp_0 += 1){
									scal_0 = input[strideA1 * (x + w) + strideA2 * (y + h) + c];
							...
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 7) + f], mem_vec_4414);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 7) + f + 16], mem_vec_4415);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 8) + f], mem_vec_4416);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 8) + f + 16], mem_vec_4417);
					}
				}
		}
}
