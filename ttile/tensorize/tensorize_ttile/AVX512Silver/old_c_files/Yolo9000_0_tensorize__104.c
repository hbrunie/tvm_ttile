...
fp_0 = 0;
						for (h = h16, hp_0 = 0;h < h16 + 3;h += 1, hp_0 += 1){
							...
							// T (x, 1) (1 / 1)
							for (x = x44, xp_2 = x44_p_1, xp_1 = x44_p_0, xp_0 = 0;x < x44 + 1;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										mem_vec_108 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * y + f]);
										mem_vec_109 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f]);
										mem_vec_110 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f]);
										mem_vec_111 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f]);
										// y = 4, x = 1, h = 1, w = 3, c = 3, f = 16
										// T (c, 3) (3 / 1)
										for (c = c26, cp_0 = 0;c < c26 + 3;c += 1, cp_0 += 1){
									...
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f], mem_vec_109);
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f], mem_vec_110);
