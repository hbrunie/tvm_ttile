...
[V f; U (1, f); U (7, y); U (3, w); T (32, c); Hoist_vars [c]; T (3, h);
  T (14, x); T (2, y); T (32, f); T (16, c); T (1, f)]
*/
IND_TYPE c, cp_0, c125_p_0, cp_1, c125, f, fp_0, f73_p_0, fp_1, f73, h, hp_0, x, xp_0, y, yp_0;
IND_TYPE y66 = 0;
IND_TYPE x66 = 0;
IND_TYPE h50 = 0;
IND_TYPE w = 0;
...
xp_0 = 0;
					for (h = h50, hp_0 = 0;h < h50 + 3;h += 1, hp_0 += 1){
								...
								mem_vec_725 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f]);
								mem_vec_726 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f]);
								mem_vec_727 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 6) + f]);
								// y = 7, x = 1, h = 1, w = 3, c = 32, f = 16
								// T (c, 32) (32 / 1)
								for (c = c125, cp_1 = c125_p_0, cp_0 = 0;c < c125 + 32;c += 1, cp_1 += 1, cp_0 += 1){
							...
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f], mem_vec_725);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f], mem_vec_726);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 6) + f], mem_vec_727);
					}
}
