...
[V f; U (2, f); ULambda y; T (256, c); Hoist_vars [c]; T (2, x); T (2, c);
  ...
  T (8, f); T (1, x); T (1, y)]
*/
IND_TYPE c, cp_0, c309_p_0, cp_1, c309, f, fp_0, x, xp_0, x412_p_0, x413_p_0, xp_1, x412_p_1, xp_2, x412, x413, y, yp_0, y283_p_0, yp_1, y283;
IND_TYPE y284 = 0;
IND_TYPE x414 = 0;
IND_TYPE h = 0;
IND_TYPE w = 0;
...
x413_p_0 = 0;
		for (f = f206, fp_0 = 0;f < f206 + 256;f += 32, fp_0 += 32){
				for (y = y283, yp_1 = y283_p_0, yp_0 = 0;y < y283 + 7;y += 7, yp_1 += 7, yp_0 += 7){
				...
				for (y = y283 + 7, yp_1 = y283_p_0, yp_0 = 0;y < y283 + 7 + 27;y += 9, yp_1 += 9, yp_0 += 9){
					// y = ph_y, x = 34, h = 1, w = 1, c = 512, f = 32
					// T (x, 17) (34 / 2)
					for (x412 = x413, x412_p_1 = x413_p_0, x412_p_0 = 0;x412 < x413 + 34;x412 += 2, x412_p_1 += 2, x412_p_0 += 2){
						// y = ph_y, x = 2, h = 1, w = 1, c = 512, f = 32
						...
						for (c309 = c310, c309_p_0 = 0;c309 < c310 + 512;c309 += 256, c309_p_0 += 256){
							...
							for (x = x412, xp_2 = x412_p_1, xp_1 = x412_p_0, xp_0 = 0;x < x412 + 2;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_4602 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 8) + f]);
										mem_vec_4603 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 8) + f + 16]);
										// y = ph_y, x = 1, h = 1, w = 1, c = 256, f = 32
										// T (c, 256) (256 / 1)
										for (c = c309, cp_1 = c309_p_0, cp_0 = 0;c < c309 + 256;c += 1, cp_1 += 1, cp_0 += 1){
											scal_0 = input[strideA1 * (x + w) + strideA2 * (y + h) + c];
									...
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 8) + f], mem_vec_4602);
									_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 8) + f + 16], mem_vec_4603);
							}
						}
					}
				}
