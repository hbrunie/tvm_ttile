...
[V f; U (1, f); U (7, y); U (3, h); T (128, c); Hoist_vars [c]; T (7, x);
  T (3, w); T (4, f); T (4, c); T (2, c); T (16, f)]
*/
IND_TYPE c, cp_0, c756_p_0, c757_p_0, cp_1, c756_p_1, cp_2, c756, c757, f, fp_0, f567_p_0, fp_1, f567, w, wp_0, x, xp_0;
IND_TYPE y = 0;
IND_TYPE x378 = 0;
IND_TYPE h = 0;
IND_TYPE w282 = 0;
...
fp_1 = 0;
				for (w = w282, wp_0 = 0;w < w282 + 3;w += 1, wp_0 += 1){
					...
					// T (x, 7) (7 / 1)
					for (x = x378, xp_0 = 0;x < x378 + 7;x += 1, xp_0 += 1){
								...
								mem_vec_2965 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f]);
								mem_vec_2966 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f]);
								mem_vec_2967 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 6) + f]);
								// y = 7, x = 1, h = 3, w = 1, c = 128, f = 16
								// T (c, 128) (128 / 1)
								for (c = c756, cp_2 = c756_p_1, cp_1 = c756_p_0, cp_0 = 0;c < c756 + 128;c += 1, cp_2 += 1, cp_1 += 1, cp_0 += 1){
							...
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f], mem_vec_2965);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f], mem_vec_2966);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 6) + f], mem_vec_2967);
					}
				}
}
