	...
	for (f = f198, fp_0 = 0;
		...
		for (y = y186, yp_0 = 0;
			...
			for (c360 = c361, c360_p_1 = c361_p_0, c360_p_0 = 0;
				c360 < c361 + 128;
				c360 += 16, c360_p_1 += 16, c360_p_0 += 16){
					...
					for (h = h142, hp_0 = 0;
						...
						for (w = w144, wp_0 = 0;
							...
							w += 1, wp_0 += 1){
									...
									mem_vec_2018 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 32]);
									mem_vec_2019 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 48]);
								...
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 32], mem_vec_2018);
								_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 48], mem_vec_2019);
						}
					}
				}
			}
