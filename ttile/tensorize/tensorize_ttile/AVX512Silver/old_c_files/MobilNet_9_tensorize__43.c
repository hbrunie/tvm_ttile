...
[V f; U (1, f); U (7, y); U (3, h); T (32, c); Hoist_vars [c]; T (7, x);
  T (3, w); T (4, f); T (4, c); T (8, c); T (16, f)]
*/
IND_TYPE c, cp_0, c768_p_0, c769_p_0, cp_1, c768_p_1, cp_2, c768, c769, f, fp_0, f576_p_0, fp_1, f576, w, wp_0, x, xp_0;
IND_TYPE y = 0;
IND_TYPE x384 = 0;
IND_TYPE h = 0;
IND_TYPE w288 = 0;
...
fp_1 = 0;
				for (w = w288, wp_0 = 0;w < w288 + 3;w += 1, wp_0 += 1){
					...
					// T (x, 7) (7 / 1)
					for (x = x384, xp_0 = 0;x < x384 + 7;x += 1, xp_0 += 1){
								mem_vec_2982 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * y + f]);
								mem_vec_2983 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f]);
								mem_vec_2984 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f]);
								mem_vec_2985 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f]);
								mem_vec_2986 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f]);
								mem_vec_2987 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f]);
							...
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f], mem_vec_2983);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f], mem_vec_2984);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f], mem_vec_2985);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f], mem_vec_2986);
							_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f], mem_vec_2987);
