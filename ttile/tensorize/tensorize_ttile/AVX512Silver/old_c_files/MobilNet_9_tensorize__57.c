...
[V f; U (1, f); U (7, y); U (3, w); T (128, c); Hoist_vars [c]; T (7, x);
  T (3, h); T (32, f); T (1, y); T (4, c); T (2, c); T (2, f)]
*/
IND_TYPE c, cp_0, c28_p_0, c29_p_0, cp_1, c28_p_1, cp_2, c28, c29, f, fp_0, f21_p_0, fp_1, f21, h, hp_0, x, xp_0, y, yp_0;
IND_TYPE y9 = 0;
IND_TYPE x14 = 0;
IND_TYPE h9 = 0;
IND_TYPE w = 0;
...
fp_1 = 0;
					for (h = h9, hp_0 = 0;h < h9 + 3;h += 1, hp_0 += 1){
						...
						for (x = x14, xp_0 = 0;x < x14 + 7;x += 1, xp_0 += 1){
									...
									mem_vec_51 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f]);
									mem_vec_52 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f]);
									mem_vec_53 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f]);
									mem_vec_54 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f]);
									mem_vec_55 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 6) + f]);
									// y = 7, x = 1, h = 1, w = 3, c = 128, f = 16
									// T (c, 128) (128 / 1)
									for (c = c28, cp_2 = c28_p_1, cp_1 = c28_p_0, cp_0 = 0;c < c28 + 128;c += 1, cp_2 += 1, cp_1 += 1, cp_0 += 1){
								...
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f], mem_vec_51);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f], mem_vec_52);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f], mem_vec_53);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 5) + f], mem_vec_54);
								_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 6) + f], mem_vec_55);
						}
					}
}
