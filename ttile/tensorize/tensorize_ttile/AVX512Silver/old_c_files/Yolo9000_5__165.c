				...
				for (y = y216 + 8, yp_1 = y216_p_0, yp_0 = 0;
					...
					for (x = x216, xp_1 = x216_p_0, xp_0 = 0;
						...
						x += 1, xp_1 += 1, xp_0 += 1){
								...
								mem_vec_2982 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 14) + f]);
								mem_vec_2983 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 14) + f + 16]);
								// y = ph_y, x = 1, h = 1, w = 1, c = 128, f = 32
								// T (c, 128) (128 / 1)
								for (c = c166, cp_0 = 0;
									c < c166 + 128;
							...
							_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 14) + f], mem_vec_2982);
							_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 14) + f + 16], mem_vec_2983);
					}
				}
		}
	}
