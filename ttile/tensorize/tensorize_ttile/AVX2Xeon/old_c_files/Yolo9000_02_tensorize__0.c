						...
						for (w = w122, wp_0 = 0;w < w122 + 3;w += 1, wp_0 += 1){
							...
							for (c181 = c182, c181_p_0 = 0;c181 < c182 + 3;c181 += 3, c181_p_0 += 3){
								...
								for (x = x227, xp_1 = x227_p_0, xp_0 = 0;x < x227 + 1;x += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_1600 = _mm256_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f]);
											mem_vec_1601 = _mm256_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 8]);
											mem_vec_1602 = _mm256_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 16]);
											mem_vec_1603 = _mm256_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 24]);
											// y = ph_y, x = 1, h = 1, w = 1, c = 3, f = 32
											// T (c, 3) (3 / 1)
										...
										_mm256_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f], mem_vec_1600);
										_mm256_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 8], mem_vec_1601);
										_mm256_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 16], mem_vec_1602);
										_mm256_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 24], mem_vec_1603);
								}
							}
						}
					}
