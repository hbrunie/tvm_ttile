...
/*
[V f; U (2, f); U (8, y); U (3, h); T (4, c); Hoist_vars [c]; T (7, x);
  T (3, w); T (2, c); T (2, x); T (7, y); T (8, f); T (16, c); T (4, x)]
*/
IND_TYPE c, cp_0, c72_p_0, c73_p_0, cp_1, c72_p_1, cp_2, c72, c73, f, fp_0, w, wp_0, x, xp_0, x88_p_0, x89_p_0, xp_1, x88_p_1, xp_2, x88, x89, y, yp_0;
	...
	for (c73 = c74, c73_p_0 = 0;
		...
		for (f = f76, fp_0 = 0;
			...
			for (y = y44, yp_0 = 0;
				y < y44 + 56;
				y += 8, yp_0 += 8){
				// y = 8, x = 14, h = 3, w = 3, c = 8, f = 16
				// T (x, 2) (14 / 7)
				for (x88 = x89, x88_p_1 = x89_p_0, x88_p_0 = 0;
					x88 < x89 + 14;
					x88 += 7, x88_p_1 += 7, x88_p_0 += 7){
					// y = 8, x = 7, h = 3, w = 3, c = 8, f = 16
					// T (c, 2) (8 / 4)
						...
						for (w = w44, wp_0 = 0;
							...
							for (x = x88, xp_2 = x88_p_1, xp_1 = x88_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_354 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
										mem_vec_355 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 8]);
										mem_vec_356 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
										mem_vec_357 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f + 8]);
										mem_vec_358 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
										mem_vec_359 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f + 8]);
										mem_vec_360 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
										mem_vec_361 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f + 8]);
										mem_vec_362 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
										mem_vec_363 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f + 8]);
										mem_vec_364 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
										mem_vec_365 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 8]);
										mem_vec_366 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 7) + f]);
										mem_vec_367 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 7) + f + 8]);
									...
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_354);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 8], mem_vec_355);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_356);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f + 8], mem_vec_357);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f], mem_vec_358);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f + 8], mem_vec_359);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_360);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f + 8], mem_vec_361);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f], mem_vec_362);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f + 8], mem_vec_363);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f], mem_vec_364);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 8], mem_vec_365);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 7) + f], mem_vec_366);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 7) + f + 8], mem_vec_367);
							}
						}
					}
				}
