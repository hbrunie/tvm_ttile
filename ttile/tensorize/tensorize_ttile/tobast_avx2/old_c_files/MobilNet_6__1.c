								...
								for (w = w264, wp_0 = 0;
									...
									for (x = x492, xp_1 = x492_p_0, xp_0 = 0;
										...
										x += 1, xp_1 += 1, xp_0 += 1){
												...
												mem_vec_4978 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16]);
												mem_vec_4979 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f + 24]);
												// y = ph_y, x = 1, h = 1, w = 1, c = 64, f = 32
												// T (c, 64) (64 / 1)
												for (c = c492, cp_1 = c492_p_0, cp_0 = 0;
													c < c492 + 64;
											...
											_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16], mem_vec_4978);
											_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f + 24], mem_vec_4979);
									}
								}
							}
						}
