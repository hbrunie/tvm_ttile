							...
							for (h = h276, hp_0 = 0;h < h276 + 3;h += 1, hp_0 += 1){
								...
								for (w = w268, wp_0 = 0;w < w268 + 3;w += 1, wp_0 += 1){
									...
									for (x = x498, xp_1 = x498_p_0, xp_0 = 0;x < x498 + 1;x += 1, xp_1 += 1, xp_0 += 1){
												...
												mem_vec_5050 = _mm256_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 16]);
												mem_vec_5051 = _mm256_loadu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 24]);
												// y = ph_y, x = 1, h = 1, w = 1, c = 4, f = 32
												// T (c, 4) (4 / 1)
											...
											_mm256_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 16], mem_vec_5050);
											_mm256_storeu_ps(&output[strideO1 * x + strideO2 * (y + 4) + f + 24], mem_vec_5051);
									}
								}
							}
}
