...
for (x1009 = x1010, x1009_p_0 = 0;
	...
	for (f628 = f629, f628_p_0 = 0;
		f628 < f629 + 64;
		f628 += 32, f628_p_0 += 32){
			for (y756 = y757, y756_p_0 = 0;
				y756 < y757 + 192;
				y756 += 64, y756_p_0 += 64){
				// y = 16 * ph_y, x = 17, h = 3, w = 3, c = 32, f = 32
				// T (x, 17) (17 / 1)
				for (x1008 = x1009, x1008_p_1 = x1009_p_0, x1008_p_0 = 0;
					...
					for (c756 = c757, c756_p_0 = 0;
						...
						for (f = f628, fp_1 = f628_p_0, fp_0 = 0;
							...
							// y = 16 * ph_y, x = 1, h = 3, w = 3, c = 8, f = 32
							// T (y, 16) (16 * ph_y / ph_y)
							for (y = y756, yp_1 = y756_p_0, yp_0 = 0;
								y < y756 + 64;
								y += 4, yp_1 += 4, yp_0 += 4){
									...
									for (x = x1008, xp_2 = x1008_p_1, xp_1 = x1008_p_0, xp_0 = 0;
										...
										x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
												...
												for (c = c756, cp_1 = c756_p_0, cp_0 = 0;
													...
													vec_24 = _mm256_fmadd_ps(vec_25, vec_26, mem_vec_7016);
													mem_vec_7016 = vec_24;

													vec_28 = _mm256_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f + 8]);

													vec_27 = _mm256_fmadd_ps(vec_25, vec_28, mem_vec_7017);
													mem_vec_7017 = vec_27;

													vec_30 = _mm256_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f + 16]);

													vec_29 = _mm256_fmadd_ps(vec_25, vec_30, mem_vec_7018);
													mem_vec_7018 = vec_29;

													vec_32 = _mm256_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f + 24]);

													vec_31 = _mm256_fmadd_ps(vec_25, vec_32, mem_vec_7019);
													mem_vec_7019 = vec_31;
													scal_5 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h + 1) + c];
													vec_34 = _mm256_set1_ps(scal_5);


													vec_33 = _mm256_fmadd_ps(vec_34, vec_26, mem_vec_7020);
													mem_vec_7020 = vec_33;



													vec_35 = _mm256_fmadd_ps(vec_34, vec_28, mem_vec_7021);
													mem_vec_7021 = vec_35;



													vec_36 = _mm256_fmadd_ps(vec_34, vec_30, mem_vec_7022);
													mem_vec_7022 = vec_36;



													vec_37 = _mm256_fmadd_ps(vec_34, vec_32, mem_vec_7023);
													mem_vec_7023 = vec_37;
													scal_6 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 2 + h + 1) + c];
													vec_39 = _mm256_set1_ps(scal_6);


													vec_38 = _mm256_fmadd_ps(vec_39, vec_26, mem_vec_7024);
													mem_vec_7024 = vec_38;



													vec_40 = _mm256_fmadd_ps(vec_39, vec_28, mem_vec_7025);
													mem_vec_7025 = vec_40;



													vec_41 = _mm256_fmadd_ps(vec_39, vec_30, mem_vec_7026);
													mem_vec_7026 = vec_41;



													vec_42 = _mm256_fmadd_ps(vec_39, vec_32, mem_vec_7027);
													mem_vec_7027 = vec_42;
													scal_7 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 3 + h + 1) + c];
													vec_44 = _mm256_set1_ps(scal_7);


													vec_43 = _mm256_fmadd_ps(vec_44, vec_26, mem_vec_7028);
													mem_vec_7028 = vec_43;



													vec_45 = _mm256_fmadd_ps(vec_44, vec_28, mem_vec_7029);
													mem_vec_7029 = vec_45;



													vec_46 = _mm256_fmadd_ps(vec_44, vec_30, mem_vec_7030);
													mem_vec_7030 = vec_46;



													vec_47 = _mm256_fmadd_ps(vec_44, vec_32, mem_vec_7031);
													mem_vec_7031 = vec_47;
													scal_8 = input[(C * (Y + H - 1)) * (x + w) + C * (y + h + 2) + c];
													vec_49 = _mm256_set1_ps(scal_8);
													vec_50 = _mm256_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f]);

													vec_48 = _mm256_fmadd_ps(vec_49, vec_50, mem_vec_7016);
													mem_vec_7016 = vec_48;

													vec_52 = _mm256_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f + 8]);

													vec_51 = _mm256_fmadd_ps(vec_49, vec_52, mem_vec_7017);
													mem_vec_7017 = vec_51;

													vec_54 = _mm256_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f + 16]);

													vec_53 = _mm256_fmadd_ps(vec_49, vec_54, mem_vec_7018);
													mem_vec_7018 = vec_53;

													vec_56 = _mm256_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f + 24]);

													vec_55 = _mm256_fmadd_ps(vec_49, vec_56, mem_vec_7019);
													mem_vec_7019 = vec_55;
													scal_9 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 1 + h + 2) + c];
													vec_58 = _mm256_set1_ps(scal_9);


													vec_57 = _mm256_fmadd_ps(vec_58, vec_50, mem_vec_7020);
													mem_vec_7020 = vec_57;



													vec_59 = _mm256_fmadd_ps(vec_58, vec_52, mem_vec_7021);
													mem_vec_7021 = vec_59;



													vec_60 = _mm256_fmadd_ps(vec_58, vec_54, mem_vec_7022);
													mem_vec_7022 = vec_60;



													vec_61 = _mm256_fmadd_ps(vec_58, vec_56, mem_vec_7023);
													mem_vec_7023 = vec_61;
													scal_10 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 2 + h + 2) + c];
													vec_63 = _mm256_set1_ps(scal_10);


													vec_62 = _mm256_fmadd_ps(vec_63, vec_50, mem_vec_7024);
													mem_vec_7024 = vec_62;



													vec_64 = _mm256_fmadd_ps(vec_63, vec_52, mem_vec_7025);
													mem_vec_7025 = vec_64;



													vec_65 = _mm256_fmadd_ps(vec_63, vec_54, mem_vec_7026);
													mem_vec_7026 = vec_65;



													vec_66 = _mm256_fmadd_ps(vec_63, vec_56, mem_vec_7027);
													mem_vec_7027 = vec_66;
													scal_11 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 3 + h + 2) + c];
													vec_68 = _mm256_set1_ps(scal_11);


													vec_67 = _mm256_fmadd_ps(vec_68, vec_50, mem_vec_7028);
													mem_vec_7028 = vec_67;



													vec_69 = _mm256_fmadd_ps(vec_68, vec_52, mem_vec_7029);
													mem_vec_7029 = vec_69;



													vec_70 = _mm256_fmadd_ps(vec_68, vec_54, mem_vec_7030);
													mem_vec_7030 = vec_70;



													vec_71 = _mm256_fmadd_ps(vec_68, vec_56, mem_vec_7031);
													mem_vec_7031 = vec_71;
												}
								...
								for (w = w454, wp_0 = 0;
									...
									for (x = x1008, xp_2 = x1008_p_1, xp_1 = x1008_p_0, xp_0 = 0;
										...
										x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
												...
												mem_vec_7050 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16]);
												mem_vec_7051 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f + 24]);
												// y = ph_y, x = 1, h = 3, w = 1, c = 8, f = 32
												// T (c, 8) (8 / 1)
												for (c = c756, cp_1 = c756_p_0, cp_0 = 0;
													c < c756 + 8;
											...
											_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f + 16], mem_vec_7050);
											_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f + 24], mem_vec_7051);
									}
								}
							}
						}
