...
[V f; U (4, f); ULambda y; U (3, w); T (64, c); Hoist_vars [c]; T (1, x);
  T (1, w); T (3, h); T (1, f); T (8, c); T (14, x);
							...
							for (h = h0, hp_0 = 0;h < h0 + 3;h += 1, hp_0 += 1){
								...
								for (w = w0, wp_0 = 0;w < w0 + 3;w += 3, wp_0 += 3){
									...
									for (x = x0, xp_1 = x0_p_0, xp_0 = 0;x < x0 + 1;x += 1, xp_1 += 1, xp_0 += 1){
												...
												mem_vec_19 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * y + f 8]);
												...
												mem_vec_23 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f 8]);
												...
												mem_vec_27 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f 8]);
												...
												mem_vec_31 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f 8]);
												...
												mem_vec_35 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y ) + f 8]);
												// y = u_y, x = 1, h = 1, w = 3, c = 64, f = 64
												// T (c, 64) (64 / 1)
												for (c = c0, cp_2 = c0_p_1, cp_1 = c0_p_0, cp_0 = 0;c < c0 + 64;c += 1, cp_2 += 1, cp_1 += 1, cp_0 += 1){
													...
													mem_vec_31 = vec_23;
													scal_4 = input[strideA1 * (x + w) + strideA2 * (y  + h) + c];
											...
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * y + f 8], mem_vec_19);
											...
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 1) + f 8], mem_vec_23);
											...
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 2) + f 8], mem_vec_27);
											...
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 3) + f 8], mem_vec_31);
											...
											_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y ) + f 8], mem_vec_35);
									}
								}
							}
}
