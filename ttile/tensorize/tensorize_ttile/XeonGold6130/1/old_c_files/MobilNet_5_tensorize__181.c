						...
						for (w = w134, wp_0 = 0;w < w134 + 3;w += 1, wp_0 += 1){
							...
							for (x = x316, xp_2 = x316_p_1, xp_1 = x316_p_0, xp_0 = 0;x < x316 + 7;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
								...
								for (h = h194, hp_0 = 0;h < h194 + 3;h += 1, hp_0 += 1){
											...
											mem_vec_2466 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f]);
											mem_vec_2467 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16]);
										...
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f], mem_vec_2466);
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 13) + f + 16], mem_vec_2467);
								}
							}
						}
}
