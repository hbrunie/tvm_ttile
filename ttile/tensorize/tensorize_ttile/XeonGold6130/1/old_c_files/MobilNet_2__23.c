	...
	x671 += 14, x671_p_0 += 14){
		for (y = y377, yp_0 = 0;
			...
			for (x670 = x671, x670_p_1 = x671_p_0, x670_p_0 = 0;
				x670 < x671 + 14;
				x670 += 14, x670_p_1 += 14, x670_p_0 += 14){
				// y = ph_y, x = 14, h = 3, w = 3, c = 64, f = 64
				// T (f, 2) (64 / 32)
				for (f = f299, fp_0 = 0;
					f < f299 + 64;
							...
							for (x = x670, xp_2 = x670_p_1, xp_1 = x670_p_0, xp_0 = 0;
								...
								for (h = h305, hp_0 = 0;
									...
									h += 1, hp_0 += 1){
											...
											mem_vec_5910 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f]);
											mem_vec_5911 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16]);
											// y = ph_y, x = 1, h = 1, w = 1, c = 32, f = 32
											// T (c, 32) (32 / 1)
											for (c = c534, cp_1 = c534_p_0, cp_0 = 0;
												c < c534 + 32;
										...
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f], mem_vec_5910);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16], mem_vec_5911);
								}
							}
						}
					}
