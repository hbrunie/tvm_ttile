						...
						for (w = w40, wp_0 = 0;w < w40 + 3;w += 1, wp_0 += 1){
							...
							for (x = x80, xp_2 = x80_p_1, xp_1 = x80_p_0, xp_0 = 0;x < x80 + 34;x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
								...
								for (h = h40, hp_0 = 0;h < h40 + 3;h += 1, hp_0 += 1){
											...
											mem_vec_992 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f]);
											mem_vec_993 = _mm512_loadu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f + 16]);
										...
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f], mem_vec_992);
										_mm512_storeu_ps(&output[strideO1 * x + strideO2 * (y + 9) + f + 16], mem_vec_993);
								}
							}
						}
}
