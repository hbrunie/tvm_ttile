				...
				for (f = f68, fp_0 = 0;
					...
					for (y = y136, yp_2 = y136_p_1, yp_1 = y136_p_0, yp_0 = 0;
						...
						for (x170 = x171, x170_p_2 = x171_p_1, x170_p_1 = x171_p_0, x170_p_0 = 0;
							x170 < x171 + 2;
							x170 += 2, x170_p_2 += 2, x170_p_1 += 2, x170_p_0 += 2){
							// y = 14, x = 2, h = 1, w = 1, c = 64, f = 32
							// T (c, 2) (64 / 32)
							for (c102 = c103, c102_p_0 = 0;
								...
								for (x = x170, xp_3 = x170_p_2, xp_2 = x170_p_1, xp_1 = x170_p_0, xp_0 = 0;
									...
									x += 1, xp_3 += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_578 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f]);
											mem_vec_579 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16]);
											mem_vec_580 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
											mem_vec_581 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
											mem_vec_582 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f]);
											mem_vec_583 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16]);
											...
											for (c = c102, cp_1 = c102_p_0, cp_0 = 0;
												...
												vec_35 = _mm512_fmadd_ps(vec_36, vec_2, mem_vec_578);
												mem_vec_578 = vec_35;



												vec_37 = _mm512_fmadd_ps(vec_36, vec_4, mem_vec_579);
												mem_vec_579 = vec_37;
												scal_12 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 12 + h) + c];
												vec_39 = _mm512_set1_ps(scal_12);


												vec_38 = _mm512_fmadd_ps(vec_39, vec_2, mem_vec_580);
												mem_vec_580 = vec_38;



												vec_40 = _mm512_fmadd_ps(vec_39, vec_4, mem_vec_581);
												mem_vec_581 = vec_40;
												scal_13 = input[(C * (Y + H - 1)) * (x + w) + C * (y + 13 + h) + c];
												vec_42 = _mm512_set1_ps(scal_13);


												vec_41 = _mm512_fmadd_ps(vec_42, vec_2, mem_vec_582);
												mem_vec_582 = vec_41;



												vec_43 = _mm512_fmadd_ps(vec_42, vec_4, mem_vec_583);
												mem_vec_583 = vec_43;
											}
										...
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f], mem_vec_578);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16], mem_vec_579);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_580);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_581);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f], mem_vec_582);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 13) + f + 16], mem_vec_583);
								}
							}
						}
					}
