...
for (y129 = y130, y129_p_0 = 0;
	...
	for (f = f81, fp_0 = 0;
		...
		f += 32, fp_0 += 32){
			...
			for (y = y129 + 8, yp_1 = y129_p_0, yp_0 = 0;
				y < y129 + 8 + 104;
							...
							for (c129 = c130, c129_p_0 = 0;
								...
								for (x = x124, xp_1 = x124_p_0, xp_0 = 0;
									...
									x += 1, xp_1 += 1, xp_0 += 1){
											...
											mem_vec_1327 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16]);
											mem_vec_1328 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
											mem_vec_1329 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
											// y = ph_y, x = 1, h = 1, w = 1, c = 3, f = 32
											// T (c, 3) (3 / 1)
											for (c = c129, cp_1 = c129_p_0, cp_0 = 0;
												c < c129 + 3;
										...
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16], mem_vec_1327);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_1328);
										_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_1329);
								}
							}
						}
					}
