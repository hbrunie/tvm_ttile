	...
	for (x85 = x86, x85_p_0 = 0;
		...
		for (f = f42, fp_0 = 0;
			...
			f += 32, fp_0 += 32){
				...
				for (y = y55 + 8, yp_1 = y55_p_0, yp_0 = 0;
					y < y55 + 8 + 26;
						...
						for (c63 = c64, c63_p_0 = 0;
							...
							for (x = x84, xp_2 = x84_p_1, xp_1 = x84_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_1002 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f]);
										mem_vec_1003 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16]);
										// y = ph_y, x = 1, h = 1, w = 1, c = 64, f = 32
										// T (c, 64) (64 / 1)
										for (c = c63, cp_1 = c63_p_0, cp_0 = 0;
											c < c63 + 64;
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f], mem_vec_1002);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 12) + f + 16], mem_vec_1003);
							}
						}
					}
				}
