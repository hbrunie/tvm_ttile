...
for (y408 = y409, y408_p_0 = 0;
	...
	for (x545 = x546, x545_p_0 = 0;
		...
		x545 += 4, x545_p_0 += 4){
			...
			for (y = y408 + 22, yp_1 = y408_p_0, yp_0 = 0;
				y < y408 + 22 + 12;
						...
						for (c408 = c409, c408_p_0 = 0;
							...
							for (x = x544, xp_2 = x544_p_1, xp_1 = x544_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										mem_vec_6482 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f]);
										mem_vec_6483 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16]);
										mem_vec_6484 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f]);
										mem_vec_6485 = _mm512_loadu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16]);
										// y = ph_y, x = 1, h = 1, w = 1, c = 64, f = 32
										// T (c, 64) (64 / 1)
										for (c = c408, cp_1 = c408_p_0, cp_0 = 0;
											c < c408 + 64;
									...
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f], mem_vec_6482);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 10) + f + 16], mem_vec_6483);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f], mem_vec_6484);
									_mm512_storeu_ps(&output[(F * Y) * x + F * (y + 11) + f + 16], mem_vec_6485);
							}
						}
					}
				}
