...
def intrin_conv(name_function, W, H, C, F, X, Y, stride_w, stride_h):
    ...
    Ab = tvm.tir.decl_buffer(a.shape, a.dtype, name="Ab", offset_factor=1, strides=[strideA1, strideA2, strideA3, 1])
    Ww = tvm.tir.decl_buffer(w.shape, w.dtype, name="Ww", offset_factor=1, strides=[strideB1, strideB2, strideB3, 1])
    Oo = tvm.tir.decl_buffer(o.shape, o.dtype, name="Oo", offset_factor=1, strides=[strideC1, strideC2, strideC3, 1])
...
def conv2d_ttile_2kernel(name, batch_size, width, height, kernel_w, kernel_h, in_channels, out_channels, info_tile, stride_w, stride_h):
    ...
    print("out_h", out_h1, out_h2, out_h1 + out_h2 , out_h)
