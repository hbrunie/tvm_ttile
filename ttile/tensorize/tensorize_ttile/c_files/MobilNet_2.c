							...
							for (x = x0, xp_2 = x0_p_1, xp_1 = x0_p_0, xp_0 = 0;
								...
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										...
										for (c = c0, cp_1 = c0_p_0, cp_0 = 0;
											...
											vec_19 = _mm256_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f]);

											vec_17 = _mm256_fmadd_ps(vec_18, vec_19, mem_vec_0);
											mem_vec_0 = vec_17;

											vec_21 = _mm256_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 1) + F * c + f + 8]);
											...
											vec_22 = _mm256_fmadd_ps(vec_23, vec_19, mem_vec_2);
											mem_vec_2 = vec_22;



											vec_24 = _mm256_fmadd_ps(vec_23, vec_21, mem_vec_3);
											mem_vec_3 = vec_24;
											scal_7 = input[(C * (2 * Y + H - 1)) * (2 * x + w) + C * (2 * (y + 2) + h + 1) + c];
											vec_26 = _mm256_set1_ps(scal_7);


											vec_25 = _mm256_fmadd_ps(vec_26, vec_19, mem_vec_4);
											mem_vec_4 = vec_25;



											vec_27 = _mm256_fmadd_ps(vec_26, vec_21, mem_vec_5);
											mem_vec_5 = vec_27;
											scal_8 = input[(C * (2 * Y + H - 1)) * (2 * x + w) + C * (2 * (y + 3) + h + 1) + c];
											vec_29 = _mm256_set1_ps(scal_8);


											vec_28 = _mm256_fmadd_ps(vec_29, vec_19, mem_vec_6);
											mem_vec_6 = vec_28;



											vec_30 = _mm256_fmadd_ps(vec_29, vec_21, mem_vec_7);
											mem_vec_7 = vec_30;
											scal_9 = input[(C * (2 * Y + H - 1)) * (2 * x + w) + C * (2 * (y + 4) + h + 1) + c];
											vec_32 = _mm256_set1_ps(scal_9);


											vec_31 = _mm256_fmadd_ps(vec_32, vec_19, mem_vec_8);
											mem_vec_8 = vec_31;



											vec_33 = _mm256_fmadd_ps(vec_32, vec_21, mem_vec_9);
											mem_vec_9 = vec_33;
											scal_10 = input[(C * (2 * Y + H - 1)) * (2 * x + w) + C * (2 * y + h + 2) + c];
											vec_35 = _mm256_set1_ps(scal_10);
											vec_36 = _mm256_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f]);

											vec_34 = _mm256_fmadd_ps(vec_35, vec_36, mem_vec_0);
											mem_vec_0 = vec_34;

											vec_38 = _mm256_loadu_ps(&params[((F * C) * H) * w + (F * C) * (h + 2) + F * c + f + 8]);
											...
											vec_39 = _mm256_fmadd_ps(vec_40, vec_36, mem_vec_2);
											mem_vec_2 = vec_39;



											vec_41 = _mm256_fmadd_ps(vec_40, vec_38, mem_vec_3);
											mem_vec_3 = vec_41;
											scal_12 = input[(C * (2 * Y + H - 1)) * (2 * x + w) + C * (2 * (y + 2) + h + 2) + c];
											vec_43 = _mm256_set1_ps(scal_12);


											vec_42 = _mm256_fmadd_ps(vec_43, vec_36, mem_vec_4);
											mem_vec_4 = vec_42;



											vec_44 = _mm256_fmadd_ps(vec_43, vec_38, mem_vec_5);
											mem_vec_5 = vec_44;
											scal_13 = input[(C * (2 * Y + H - 1)) * (2 * x + w) + C * (2 * (y + 3) + h + 2) + c];
											vec_46 = _mm256_set1_ps(scal_13);


											vec_45 = _mm256_fmadd_ps(vec_46, vec_36, mem_vec_6);
											mem_vec_6 = vec_45;



											vec_47 = _mm256_fmadd_ps(vec_46, vec_38, mem_vec_7);
											mem_vec_7 = vec_47;
											scal_14 = input[(C * (2 * Y + H - 1)) * (2 * x + w) + C * (2 * (y + 4) + h + 2) + c];
											vec_49 = _mm256_set1_ps(scal_14);


											vec_48 = _mm256_fmadd_ps(vec_49, vec_36, mem_vec_8);
											mem_vec_8 = vec_48;



											vec_50 = _mm256_fmadd_ps(vec_49, vec_38, mem_vec_9);
											mem_vec_9 = vec_50;
										}
					...
					for (c0 = c1, c0_p_0 = 0;
						...
						for (w = w0, wp_0 = 0;
							...
							for (x = x0, xp_2 = x0_p_1, xp_1 = x0_p_0, xp_0 = 0;
								x < x0 + 1;
								x += 1, xp_2 += 1, xp_1 += 1, xp_0 += 1){
										mem_vec_10 = _mm256_loadu_ps(&output[(F * Y) * x + F * y + f]);
										mem_vec_11 = _mm256_loadu_ps(&output[(F * Y) * x + F * y + f + 8]);
										mem_vec_12 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f]);
										mem_vec_13 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 1) + f + 8]);
										mem_vec_14 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f]);
										mem_vec_15 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 2) + f + 8]);
										mem_vec_16 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f]);
										mem_vec_17 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 3) + f + 8]);
										mem_vec_18 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f]);
										mem_vec_19 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 4) + f + 8]);
										mem_vec_20 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f]);
										mem_vec_21 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 5) + f + 8]);
										mem_vec_22 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f]);
										mem_vec_23 = _mm256_loadu_ps(&output[(F * Y) * x + F * (y + 6) + f + 8]);
										// y = u_y, x = 1, h = 3, w = 1, c = 1, f = 16
										// T (c, 1) (1 / 1)
										for (c = c0, cp_1 = c0_p_0, cp_0 = 0;
											c < c0 + 1;
									...
									_mm256_storeu_ps(&output[(F * Y) * x + F * y + f], mem_vec_10);
									_mm256_storeu_ps(&output[(F * Y) * x + F * y + f + 8], mem_vec_11);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f], mem_vec_12);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 1) + f + 8], mem_vec_13);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f], mem_vec_14);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 2) + f + 8], mem_vec_15);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f], mem_vec_16);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 3) + f + 8], mem_vec_17);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f], mem_vec_18);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 4) + f + 8], mem_vec_19);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f], mem_vec_20);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 5) + f + 8], mem_vec_21);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f], mem_vec_22);
									_mm256_storeu_ps(&output[(F * Y) * x + F * (y + 6) + f + 8], mem_vec_23);
							}
						}
					}
				}
