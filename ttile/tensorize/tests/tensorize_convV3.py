...
def generate_ttile_conv2d(order, unroll):
...
extern "C" {{
    ...
    int Mttile_conv2d(float* K, float * Input, float * Output, int W, int H, int C, int F, int X, int Y, int strideO1, int strideO2, int strideA1, int strideA2, int strideW1, int strideW2, int strideW3) {{
        ...
        */
        {unroll[0]}
        for (int {order[0].lower()}=0; {order[0].lower()}<{order[0].upper()}; {order[0].lower()}++){{
            {unroll[1]}
            for (int {order[1].lower()}=0; {order[1].lower()}<{order[1].upper()}; {order[1].lower()}++){{
                {unroll[2]}
                for (int {order[2].lower()}=0; {order[2].lower()}<{order[2].upper()}; {order[2].lower()}++){{
                    {unroll[3]}
                    for (int {order[3].lower()}=0; {order[3].lower()}<{order[3].upper()}; {order[3].lower()}++){{
                        {unroll[4]}
                        for (int {order[4].lower()}=0; {order[4].lower()}<{order[4].upper()}; {order[4].lower()}++){{
                            {unroll[5]}
                            for (int {order[5].lower()}=0; {order[5].lower()}<{order[5].upper()}; {order[5].lower()}++){{
...
def conv_impl(order, unroll):

    unroll_ = []

    for k in range(len(unroll)):
        if unroll[k] != str(1):
            unroll_ += ["#pragma unroll(" + unroll[k] + ")"]
        else:
            unroll_ += [""]

    cc_code = generate_ttile_conv2d(order, unroll_)
...
s[Out].pragma(axe_batch, "import_llvm", conv_impl(order, unroll))
