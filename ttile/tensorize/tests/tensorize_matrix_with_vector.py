...
// not optimize at all but just an example

int gemv_update(float *cc, float *aa, float *bb, int n, int m, int l, int strideA, int strideB, int strideC) {
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            for (int k = 0; k < l; k+=8){
                __m256 vA = _mm256_load_ps(&aa[i * strideA + k]);
                __m256 vB = _mm256_load_ps(&bb[j * strideB + k]);
                vA = vA * vB;
                __m128 vC1 = _mm256_extractf128_ps(vA, 0);
                __m128 vC2 = _mm256_extractf128_ps(vA, 1);
                vC1 = vC1 + vC2;
                float t = vC1[0] + vC1[1] + vC1[2] + vC1[3];
                cc[i * strideC + j] += t;
            }
        }
    }
...
int gemv_reset(float *cc, int n, int m, int strideC) {
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; j+=8){
            __m256 vZ = _mm256_setzero_ps();
            _mm256_store_ps(&cc[i * strideC + j], vZ);
        }
    }
    return 0;
}
...
three = tvm.nd.array(np.ones(get_const_tuple(C.shape), dtype=dtype) * 3, ctx)
