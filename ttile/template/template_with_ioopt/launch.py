...
# for name_input in ["MobilNet-8"]:
    generate_template.generate_template(name_input, target, archi, "up")
    # generate_template.generate_apply_best(name_input, target, archi, "up")
    template_name = name_input.replace("-", "_")
    exec("import template_%s" % (template_name))
    exec("template_%s.evaluate()" % (template_name))

    generate_template.generate_template(name_input, target, archi, "down")
    # generate_template.generate_apply_best(name_input, target, archi, "down")
    template_name = name_input.replace("-", "_")
    exec("import template_%s" % (template_name))
    exec("template_%s.evaluate()" % (template_name))

    generate_template.generate_template(name_input, target, archi, "closest")
    # generate_template.generate_apply_best(name_input, target, archi, "closest")
    template_name = name_input.replace("-", "_")
    exec("import template_%s" % (template_name))
    exec("template_%s.evaluate()" % (template_name))
