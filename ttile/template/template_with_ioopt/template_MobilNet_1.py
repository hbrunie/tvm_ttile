...
def conv2d_ttile_MobilNet_1(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
    ...
    cfg.define_knob("unroll_axe_kernel_w", [True, False])
        
    cfg.define_knob("unroll_axe_kernel_h", [True, False])
    ...
    if cfg["unroll_axe_kernel_w"].val:
        s[Out].unroll(axe_kernel_w)
        
    if cfg["unroll_axe_kernel_h"].val:
        s[Out].unroll(axe_kernel_h)
