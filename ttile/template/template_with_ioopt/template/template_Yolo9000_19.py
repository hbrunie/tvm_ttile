...
log_file = "log/autotvm_conv2d_Yolo9000-19.log"
...
@autotvm.template("conv2d_ttile_Yolo9000_19")
def conv2d_ttile_Yolo9000_19(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
    ...
    axe_out_channels_1, axe_out_channels_0 = s[Out].split(axe_out_channels, 69)
                
    axe_in_channels_1, axe_in_channels_0 = s[Out].split(axe_in_channels, 311)
                
    axe_xx_1, axe_xx_0 = s[Out].split(axe_xx, 14)
                
    axe_xx_2, axe_xx_1 = s[Out].split(axe_xx_1, 14)
                
    axe_yy_1, axe_yy_0 = s[Out].split(axe_yy, 14)
                
    axe_yy_2, axe_yy_1 = s[Out].split(axe_yy_1, 14)
                
    s[Out].reorder(axe_yy_1,axe_xx_1,axe_in_channels_1,axe_out_channels_1,axe_in_channels_0,axe_yy_0,axe_xx_0,axe_out_channels_0)
...
def evaluate():
    task = autotvm.task.create("conv2d_ttile_Yolo9000_19", args=(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w), target=target)
    ...
    with autotvm.apply_history_best(log_file):
        with tvm.target.Target(target):
            s, arg_bufs = conv2d_ttile_Yolo9000_19(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w)
    ...
    # for k in range(19):
    #     time += [float(os.popen("python3 template/apply_best_Yolo9000_19.py").read())]
    ...
    # f = open("result.txt", "a")
    # f.write("Yolo9000_19 " + str(result) + " " + str(std) + "\n")
    # f.close()

    f = open("result.txt", "a")
    f.write("Yolo9000_19 " + str(result) + "\n")
    f.close()
