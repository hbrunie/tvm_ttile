...
log_file = "log/autotvm_conv2d_ResNet18-6.log"
...
@autotvm.template("conv2d_ttile_ResNet18_6")
def conv2d_ttile_ResNet18_6(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
...
def evaluate():
    task = autotvm.task.create("conv2d_ttile_ResNet18_6", args=(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w), target=target)
    ...
    with autotvm.apply_history_best(log_file):
        with tvm.target.Target(target):
            s, arg_bufs = conv2d_ttile_ResNet18_6(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w)
    ...
    # for k in range(19):
    #     time += [float(os.popen("python3 template/apply_best_ResNet18_6.py").read())]
    ...
    # f = open("result.txt", "a")
    # f.write("ResNet18_6 " + str(result) + " " + str(std) + "\n")
    # f.close()

    f = open("result.txt", "a")
    f.write("ResNet18_6 " + str(result) + "\n")
    f.close()
