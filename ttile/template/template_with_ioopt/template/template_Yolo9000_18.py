...
log_file = "log/autotvm_conv2d_Yolo9000-18.log"
...
@autotvm.template("conv2d_ttile_Yolo9000_18")
def conv2d_ttile_Yolo9000_18(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
    ...
    axe_out_channels_1, axe_out_channels_0 = s[Out].split(axe_out_channels, 27)
                
    axe_in_channels_1, axe_in_channels_0 = s[Out].split(axe_in_channels, 116)
                
    axe_in_channels_2, axe_in_channels_1 = s[Out].split(axe_in_channels_1, 116)
                
    s[Out].reorder(axe_in_channels_1,axe_out_channels_1,axe_in_channels_0,axe_kernel_h,axe_kernel_w,axe_yy,axe_xx,axe_out_channels_0)
    
    cfg.define_knob("unroll_axe_out_channels_0", [True, False])
        
    cfg.define_knob("unroll_axe_xx", [True, False])
        
    cfg.define_knob("unroll_axe_yy", [True, False])
...
def evaluate():
    task = autotvm.task.create("conv2d_ttile_Yolo9000_18", args=(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w), target=target)
    ...
    with autotvm.apply_history_best(log_file):
        with tvm.target.Target(target):
            s, arg_bufs = conv2d_ttile_Yolo9000_18(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w)
    ...
    # for k in range(19):
    #     time += [float(os.popen("python3 template/apply_best_Yolo9000_18.py").read())]
    ...
    # f = open("result.txt", "a")
    # f.write("Yolo9000_18 " + str(result) + " " + str(std) + "\n")
    # f.close()

    f = open("result.txt", "a")
    f.write("Yolo9000_18 " + str(result) + "\n")
    f.close()
