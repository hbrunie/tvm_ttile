...
log_file = "log/autotvm_conv2d_ResNet18-8.log"
...
@autotvm.template("conv2d_ttile_ResNet18_8")
def conv2d_ttile_ResNet18_8(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
    ...
    axe_in_channels_1, axe_in_channels_0 = s[Out].split(axe_in_channels, 93)
                
    axe_xx_1, axe_xx_0 = s[Out].split(axe_xx, 28)
                
    axe_xx_2, axe_xx_1 = s[Out].split(axe_xx_1, 23)
...
def evaluate():
    task = autotvm.task.create("conv2d_ttile_ResNet18_8", args=(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w), target=target)
    ...
    with autotvm.apply_history_best(log_file):
        with tvm.target.Target(target):
            s, arg_bufs = conv2d_ttile_ResNet18_8(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w)
    ...
    # for k in range(19):
    #     time += [float(os.popen("python3 template/apply_best_ResNet18_8.py").read())]
    ...
    # f = open("result.txt", "a")
    # f.write("ResNet18_8 " + str(result) + " " + str(std) + "\n")
    # f.close()

    f = open("result.txt", "a")
    f.write("ResNet18_8 " + str(result) + "\n")
    f.close()
