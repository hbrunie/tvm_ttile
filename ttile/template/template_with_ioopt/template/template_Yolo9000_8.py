...
def conv2d_ttile_Yolo9000_8(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
    ...
    axe_out_channels_1, axe_out_channels_0 = s[Out].split(axe_out_channels, 113)
                
    axe_out_channels_2, axe_out_channels_1 = s[Out].split(axe_out_channels_1, 14)
                
    axe_in_channels_1, axe_in_channels_0 = s[Out].split(axe_in_channels, 92)
                
    axe_xx_1, axe_xx_0 = s[Out].split(axe_xx, 10)
                
    axe_yy_1, axe_yy_0 = s[Out].split(axe_yy, 18)
...
def evaluate():
    task = autotvm.task.create("conv2d_ttile_Yolo9000_8", args=(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w), target=target)
    ...
    # for k in range(19):
    #     time += [float(os.popen("python3 template/apply_best_Yolo9000_8.py").read())]
    ...
    # f = open("result.txt", "a")
    # f.write("Yolo9000_8 " + str(result) + " " + str(std) + "\n")
    # f.close()

    f = open("result.txt", "a")
    f.write("Yolo9000_8 " + str(result) + "\n")
    f.close()
