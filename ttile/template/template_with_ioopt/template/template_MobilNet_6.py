...
log_file = "log/autotvm_conv2d_MobilNet-6.log"
...
@autotvm.template("conv2d_ttile_MobilNet_6")
def conv2d_ttile_MobilNet_6(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
    ...
    axe_out_channels_1, axe_out_channels_0 = s[Out].split(axe_out_channels, 155)
                
    axe_out_channels_2, axe_out_channels_1 = s[Out].split(axe_out_channels_1, 68)
                
    axe_in_channels_1, axe_in_channels_0 = s[Out].split(axe_in_channels, 50)
...
def evaluate():
    task = autotvm.task.create("conv2d_ttile_MobilNet_6", args=(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w), target=target)
    ...
    with autotvm.apply_history_best(log_file):
        with tvm.target.Target(target):
            s, arg_bufs = conv2d_ttile_MobilNet_6(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w)
    ...
    # for k in range(19):
    #     time += [float(os.popen("python3 template/apply_best_MobilNet_6.py").read())]
    ...
    # f = open("result.txt", "a")
    # f.write("MobilNet_6 " + str(result) + " " + str(std) + "\n")
    # f.close()

    f = open("result.txt", "a")
    f.write("MobilNet_6 " + str(result) + "\n")
    f.close()
