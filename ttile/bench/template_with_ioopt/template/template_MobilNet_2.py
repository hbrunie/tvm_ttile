...
log_file = "log/autotvm_conv2d_MobilNet-2.log"
...
@autotvm.template("conv2d_ttile_MobilNet_2")
def conv2d_ttile_MobilNet_2(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
    ...
    axe_in_channels_1, axe_in_channels_0 = s[Out].split(axe_in_channels, 52)
                
    axe_xx_1, axe_xx_0 = s[Out].split(axe_xx, 8)
                
    axe_yy_1, axe_yy_0 = s[Out].split(axe_yy, 56)
                
    axe_yy_2, axe_yy_1 = s[Out].split(axe_yy_1, 14)
                
    s[Out].reorder(axe_yy_1,axe_xx_1,axe_in_channels_0,axe_kernel_h,axe_kernel_w,axe_yy_0,axe_xx_0,axe_out_channels)
...
def evaluate():
    task = autotvm.task.create("conv2d_ttile_MobilNet_2", args=(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w), target=target)
    ...
    with autotvm.apply_history_best(log_file):
        with tvm.target.Target(target):
            s, arg_bufs = conv2d_ttile_MobilNet_2(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w)
    ...
    # for k in range(19):
    #     time += [float(os.popen("python3 template/apply_best_MobilNet_2.py").read())]
    ...
    # f = open("result.txt", "a")
    # f.write("MobilNet_2 " + str(result) + " " + str(std) + "\n")
    # f.close()

    f = open("result.txt", "a")
    f.write("MobilNet_2 " + str(result) + "\n")
    f.close()
