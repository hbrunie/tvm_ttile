...
def conv2d_ttile_MobilNet_1(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
    ...
    axe_out_channels_1, axe_out_channels_0 = s[Out].split(axe_out_channels, 23)
                
    axe_xx_1, axe_xx_0 = s[Out].split(axe_xx, 15)
                
    axe_yy_1, axe_yy_0 = s[Out].split(axe_yy, 9)
...
def evaluate():
    ...
    # for k in range(19):
    #     time += [float(os.popen("python3 template/apply_best_MobilNet_1.py").read())]
    ...
    # f = open("result.txt", "a")
    # f.write("MobilNet_1 " + str(result) + " " + str(std) + "\n")
    # f.close()

    f = open("result.txt", "a")
    f.write("MobilNet_1 " + str(result) + "\n")
    f.close()
