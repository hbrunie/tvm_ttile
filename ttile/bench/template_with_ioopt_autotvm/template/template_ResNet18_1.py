...
log_file = "log/autotvm_conv2d_ResNet18-1.log"
...
@autotvm.template("conv2d_ttile_ResNet18_1")
def conv2d_ttile_ResNet18_1(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
    ...
    cfg.define_knob("tile_out_h_1", [k for k in range(1, out_h + 1) if out_h%k == 0])
            
    cfg.define_knob("tile_out_h_2", [k for k in range(1, out_h + 1) if out_h%k == 0])
            
    axe_xx_1, axe_xx_0 = s[Out].split(axe_xx, cfg["tile_out_w_0"].val)
                
    axe_yy_1, axe_yy_0 = s[Out].split(axe_yy, cfg["tile_out_h_0"].val)
                
    axe_yy_2, axe_yy_1 = s[Out].split(axe_yy_1, cfg["tile_out_h_1"].val)
                
    axe_yy_3, axe_yy_2 = s[Out].split(axe_yy_2, cfg["tile_out_h_2"].val)
                
    s[Out].reorder(axe_yy_2,axe_xx_1,axe_yy_1,axe_out_channels,axe_kernel_h,axe_kernel_w,axe_yy_0,axe_xx_0,axe_in_channels)
    
    cfg.define_knob("unroll_axe_in_channels", [True, False])
        
    cfg.define_knob("unroll_axe_xx_0", [True, False])
        
    if cfg["unroll_axe_in_channels"].val:
        s[Out].unroll(axe_in_channels)
        
    if cfg["unroll_axe_xx_0"].val:
        s[Out].unroll(axe_xx_0)
        
    #s[Out].vectorize(axe_in_channels)
    s[Out].vectorize(axe_out_channels)
...
def evaluate():
    task = autotvm.task.create("conv2d_ttile_ResNet18_1", args=(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w), target=target)
    ...
    with autotvm.apply_history_best(log_file):
        with tvm.target.Target(target):
            s, arg_bufs = conv2d_ttile_ResNet18_1(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w)
    ...
    for k in range(19):
        time += [float(os.popen("python3 template/apply_best_ResNet18_1.py").read())]
    ...
    f = open("result.txt", "a")
    f.write("ResNet18_1 " + str(result) + " " + str(std) + "\n")
    f.close()
