...
log_file = "log/autotvm_conv2d_Yolo9000-2.log"
...
@autotvm.template("conv2d_ttile_Yolo9000_2")
def conv2d_ttile_Yolo9000_2(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
...
def evaluate():
    task = autotvm.task.create("conv2d_ttile_Yolo9000_2", args=(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w), target=target)
    ...
    with autotvm.apply_history_best(log_file):
        with tvm.target.Target(target):
            s, arg_bufs = conv2d_ttile_Yolo9000_2(batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w)
    ...
    for k in range(19):
        time += [float(os.popen("python3 template/apply_best_Yolo9000_2.py").read())]
    ...
    f = open("result.txt", "a")
    f.write("Yolo9000_2 " + str(result) + " " + str(std) + "\n")
    f.close()
