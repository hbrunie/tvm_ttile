...
extern "C" {
    ...
    file_ = open("gen_conv_mbnet3_avx512_tensorize_all.c", "r")
    ...
    void ttile_conv2d_reset(float * Output, int F, int X, int Y, int strideO1, int strideO2) {
        ...
        */
        ...
        for (int f=0; f<F; f+=16){
            for (int y=0; y<Y; y+=1){
                for (int x=0; x<X; x+=1){
                    _mm256_store_ps(&Output[x*strideO1 + y*strideO2 + f], _mm256_setzero_ps());
                    _mm256_store_ps(&Output[x*strideO1 + y*strideO2 + f], _mm256_setzero_ps());
                }
            }
        }
...
func = tvm.build(s, [A, W, Out], target="llvm -mcpu=skylake-avx512", name="conv")
...
evaluator = func.time_evaluator(func.entry_name, ctx, number=2, repeat=10)
...
# ctx = tvm.cpu(0)
# f = tvm.build(s, [A, W, Out], "llvm -mcpu=skylake-avx512")
...
o = tvm.nd.array(np.zeros(get_const_tuple(Out.shape), dtype=dtype), ctx)
def conv2d(weight, input_, output, batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w):
...
output_conv2d = conv2d(w.asnumpy(), a.asnumpy(), o.asnumpy(), batch_size, height, width, in_channels, out_channels, kernel_h, kernel_w, stride_h, stride_w)
...
print(tensorize_result)

tvm.testing.assert_allclose(output_conv2d, tensorize_result, rtol=1e-5)
